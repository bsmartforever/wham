pro mkblockavgLMC, dir, save = save, ftsext = ftsext, $
                outdir = outdir, outfile = outfile, $
                exclude = exclude, single = single, $
                nostars = nostars, fainter = fainter

  ;; Modifying to take in an array of files and average them
  ...
  IF keyword_set(single) THEN BEGIN 
    blocks = file_search(dir + '/combo/' + single + '*.fts', count = bcount)
  ENDIF ELSE BEGIN 
  ...
  ENDIF ELSE BEGIN
    ...
    FOR k = 0, n_elements(blocks)-1 DO BEGIN
      ...
      readblock, dir + '/combo/' + blocks, map, ftsext = ftsext, /extended
