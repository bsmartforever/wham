...
;-
	...
	if strmatch(files[0], '*.gz') eq 1 then compress = 1 else compress = 0
	res=execute('readcol,"' + files[0] + '"' + namestr + ', compress = compress, _extra=extra')
	FOR j=0, ncol - 1 DO res=execute(vv[j] + '=' + vvtemp[j])
	
	IF n_elements(files) GT 1 THEN FOR i=1, n_elements(files) - 1 DO BEGIN
	  if strmatch(files[i], '*.gz') eq 1 then compress = 1 else compress = 0
		res=execute('readcol,"' + files[i] + '"' + namestr + ', compress = compress, _extra=extra')
