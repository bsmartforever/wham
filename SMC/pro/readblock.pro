...
;  dir = strmid(basename, 0, last_slash+1)
  ...
  ;; set extension if not passed in
  ...
  h = headfits(sfiles[0], exten = ftsext)
  IF strtrim(h[0], 2) EQ '-1' THEN BEGIN
    message, 'Requested extension not found in ' + sfiles[0] + '; skipping block', /info
    popd
    return
  ENDIF
  ...
  FOR i = 0, n_elements(sfiles)-1 DO BEGIN
    ...
    readfspe, spectname, vp, dp, sp, ext = 'PROCSPEC', errmsg = errmsg
    ...
    IF errmsg EQ '' THEN begin
      ...
      ;; Correct to standard radio LSR frame 
      vframe = fxpar(btheader, 'velframe')
      if (!err ne -1) && (vframe.trim() eq 'LSR') then begin
        whamtolsr, map[i].glon, map[i].glat, map[i].vel, newv, $
          vvwhamsrc = vvwhamsrc, vvlsrsrc = vvlsrsrc
        map[i].vel = newv
        map[i].vlsr = map[i].vlsr - (-vvwhamsrc) + (-vvlsrsrc)
      endif
...
;                               cos(alt*!dtor))
        ...
        IF (!err NE -1) THEN BEGIN 
          ...
          map[i].atemp = fxpar(btheader, 'atemp')
