PRO plotspectrum, vel, data, chop = chop, adu = adu, rey = rey, over = over, $
                  errors = errors, hi = hi, color = color, scale = scale, _extra = e
...
;;  IF n_elements(xr) EQ 0 THEN xr = [0, 0]
  ...
  IF KEYWORD_SET(over) THEN BEGIN 
    oplot, vel[mask], plotdata[mask], thick = thick, color = color, _extra = e
  ENDIF ELSE BEGIN 
    plot, vel[mask], plotdata[mask], $
          xstyle = 3, ystyle = 3, color = color, charsize = scale, $
  ...
  oploterror, vel[mask], plotdata[mask], ploterrors[mask], psym=3, $
    thick = thick, color = color
  
;    errplot, vel[mask], $
;             plotdata[mask]-ploterrors[mask], plotdata[mask]+ploterrors[mask], _extra = e
  ENDIF 
