pro gridmap, map, outgrid, xc, yc, z = z, $
  ...
  missing = missing, method = method, verbose = verbose, $
  lon_full = lon_full, _extra = ex_keys
  ...
  if ~isa(value) then value = intmap(map, vmin=vmin, vmax=vmax)
  ...
  ;triangulate, lon, lat, triangles, /degrees, sphere = tsphere, fvalue = fvalue

  ;; Handle limits properly if we have a map across the longitude = 0 "seam"
  
  lonspan, x, xspan, lonmid = xmid, full = lon_full

  grid_min = [xspan[0], y.min()]
  grid_max = [xspan[0] + reduceto360(xspan[1] - xspan[0]), y.max()]
  ...
  outgrid = griddata(lon, lat, fvalue, /sphere, /degrees, $
...
;    bounds = [xc.min(), yc.min(), xc.max(), yc.max()], nx = xc.length, ny = yc.length) 
  ...
  xc = reverse(reduceto360(xc))
;  xc = reverse(xc, /overwrite)

  if keyword_set(verbose) then $
    print, z.min(), z.max(), outgrid.min(/nan), outgrid.max(/nan)
end
