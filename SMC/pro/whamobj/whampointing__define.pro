...
;  vel = indgen(velrange[1] - velrange[0]) + velrange[0]
  ...
  if KEYWORD_SET(rawvel) then plot_vel = raw_vel else plot_vel = self.spectrum.vel
...
;      The FITS extension to read the pointing from.
;    chop : in, optional, type=boolean
;      Retain only the 100 right-most data points from the FITS spectrum.
;    guessvc : in, optional, type=boolean
;      Guess LSR velocity calibration from tunes and adjust velocity scale.
;      
; :Author: Matt Haffner
;-
pro WHAMPointing::Read, file, ftsext = ftsext, chop = chop, guessvc = guessvc

  if ~keyword_set(ftsext) then ftsext = 'PROCSPEC'
  ...
  if status eq 0 then begin 
    ws = WHAMSpectrum(spectrum.velocity, spectrum.data, spectrum.variance)
    if keyword_set(chop) then ws = ws[-100:-1]  ; 100 right-most data points
    self.spectrum = ws
    ...
    if (ftsext ne 'RAWSPEC') && (ftsext ne 'PROCSPEC') then begin
...
function WHAMPointing::Init, file, test = test, _extra = _extra
  ...
  if KEYWORD_SET(test) then begin 
    ...
    if n_elements(file) ne 0 then begin 
      ...
      ;; check for file read error (e.g., missing extension)
      if self.spectrum eq !null then self.spectrum = WHAMSpectrum()
    endif else begin
