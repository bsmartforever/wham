...
;  profiles[10].lambda += 22.78
  profiles[where(profiles.name.matches('^sii_blue_proposed$'))].lambda -= 12.0
  profiles[where(profiles.name.matches('^nii_proposed$'))].lambda -= 5.5
...
;      print, filters.short_name[good[i]] + ' shifted by ' + strtrim(avg(x_eff - x), 2)
  ...
  if keyword_set(thar) then begin
    ...
    oplot, thar_l, thar_s/max_in_win, color=color(250)
  endif
  ...
  if n_elements(uves) ne 0 then begin
    ...
    oplot, uves_l, uves_s/max_in_win, color=color(250)
  endif
