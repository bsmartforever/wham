PRO     gauss_noip, X, A, F, PDER
;+
; NAME:
;       gauss_noip  (based on Users' Library routine FUNCT)
;
; PURPOSE:
;       Evaluate the sum of (state.ng) Gaussians and up to a 3rd-order
;       polynomial and optionally return the value of the partial
;       derivatives.  Normally, this function is used by FITTING to
;       fit the sum of lines and a varying background to actual data.
;
; CATEGORY:
...
;  ON_ERROR, 2                   ;Return to caller if an error occurs
  ...
  COMMON fitparams, all_params, param_mask, num_bk
...
;  print, a
  ...
  IF (n_gauss GT 0) THEN BEGIN 
      FOR i = 0, (n_gauss-1) DO BEGIN
          arg = d2*((x-m(i))/w(i))^2.0
          f = f + (ar(i)/sqrt(w(i)^2.0)*d1*exp(-arg)*(abs(arg) LE 88.0)) 
      ENDFOR
  ENDIF
  ...
  IF (n_gauss GT 0) THEN BEGIN 
      FOR i = 0, (n_gauss-1) DO BEGIN
          ...
          z = (x-m(i))/w(i)     ; Time savers
          ex = exp(-d2*z^2.0)*(abs(z) LE 88.0)

          ;; Mean
          IF ((param_mask AND 2L^g_index) NE 0) THEN BEGIN
              pder(*, a_index) = 2.0*d1*d2*ar(i)/w(i)^2.0*ex*z
              a_index =  a_index + 1
          ENDIF 

          ;; Width
          IF ((param_mask AND 2L^(g_index+1)) NE 0) THEN BEGIN
              pder(*, a_index) = (2.0*d1*d2*ar(i)/w(i)^2.0*ex*z^2.0) $
                - (d1*ar(i)/w(i)^2.0*ex) 
              a_index = a_index + 1
          ENDIF
