...
;
; CALLING SEQUENCE:
;	line_edit2,x,y,mask,mask2
;
; INPUTS
;	x - input x vector
;	y - input y vector
;	mask - mask of good data (1=good, 0=bad)
;	mask2 - optional second mask of points to flag as suspect in the
;		plot.
...
;	stflags - the timeline status flags array 
;
; INTERACTIVE INPUTS:
...
;       version _   B. Godard   Nov  2003 : print IDF timeflags on
;                                           cursor (optional)
;       version 1.1 B. Godard   Aug  2004 : added binning capability
;                                           for plotting                         
;       version 2   V. Dixon    Feb  2007 : Change line_edit to line_edit2
;					throughout to prevent conflicts
;					with Lindler's new COS tools.
;-
;----------------------------------------------------------------------------  

;====================================================== LINE_EDIT_EVENT
;
; Main event driver for line_edit2
;
pro line_edit2_event,event
	common line_edit2_common,info,xarray,yarray,m,m1,m2,rsave,gsave,bsave,st_flags,existstflags
	widget_control,event.id,get_uvalue=uvalue
        widget_control,info.binsize_base,get_value=binsize
	case uvalue of
	'EXIT': begin
		xarray = 0
		yarray = 0
	...
	'WRITE': begin
		...
		'Write.Postscript File': begin
			...
			line_edit2_plot,info,xarray,yarray,m,m2,/ps
			device,/close
	...
	'UNZOOM': begin
		...
		line_edit2_plot,info,xarray,yarray,m,m2
		end
	'RANGE': line_edit2_plot,info,xarray,yarray,m,m2
	'XLOG': begin
	...
	'PLOT1': begin
		...
		if (info.state eq 'X/Y') then begin
		    ...
		    v = convert_coord(xd,yd,/dev,/to_data)
                     ...
                     if (existstflags) then begin
                     	index=value_locate(xarray, v(0))
                     	if (index GE n_elements(xarray)) then index=n_elements(xarray)-1
                     	if (index LT 0) then index=0
                     	flag=st_flags(index)
  
                     	msg='FLAGS--> '
  
                     	if ((byte(flag) and 1B) GT 0) then msg=msg+'DAY ' else msg=msg+'NIGHT '
                     	if ((byte(flag) and 2B) GT 0) then msg=msg+'LIMB_ANGLE '
                     	if ((byte(flag) and 4B) GT 0) then msg=msg+'SAA '
                     	if ((byte(flag) and 8B) GT 0) then msg=msg+'HIGH_VOLTAGE '
                     	if ((byte(flag) and 16B) GT 0) then msg=msg+'BURSTS '
                     	if ((byte(flag) and 32B) GT 0) then msg=msg+'NOT_IN_OPUS_GTI '
                     	if ((byte(flag) and 64B) GT 0) then msg=msg+'JITTER '
                     	if ((byte(flag) and 128B) GT 0) then msg=msg+'USER_DEFINED '
			...
			'X = '+strtrim(v(0),2)+'    Y = '+strtrim(v(1),2)+'    '+msg		    
...
;
pro line_edit2_plot,info,x,y,m1,m2,ps=ps

;
; create single mask 0=bad 1=good 2=suspect 
;
	mask = m1 + ((m2 eq 0) and (m1 eq 1))
...
;
	...
	while ipos lt ns do begin
		if mask(ipos) ne mask(i1) then begin
			i2 = ipos-1
                        oplot,x(i1:i2),y(i1:i2),color=colors(mask(i1)), $
                         psym=-4,symsize=symsize, nsum=binsize
			i1 = i2+1
		end
		ipos = ipos + 1
	end
	i2 = ns-1        
        oplot,x(i1:i2),y(i1:i2),color=colors(mask(i1)),psym=-4,symsize=symsize, nsum=binsize
	if not keyword_set(ps) then begin
		wset,info.pixid
...
pro line_edit2,xin,yin,mask,mask2,stflags=stflags,title=title,xtitle=xtitle,ytitle=ytitle, $
...
;
	if n_params(0) lt 1 then begin
		print,'CALLING SEQUENCE:  line_edit2,x,y,mask
	...
	if (keyword_set(stflags)) then begin 
	    existstflags=1
	    st_flags=stflags 
	endif else begin 
	    existstflags=0
...
;
	...
	yarray = yin
	m = mask
	if n_elements(mask2) gt 0 then m2 = mask2 else m2 = replicate(1b,ns)
	tvlct,r,g,b,/get
	rsave = r
	gsave = g
	bsave = b
...
;
	...
	basex = widget_base(main,/row,/frame)	
        ...
        binsize_base = cw_field(basex,/row,uvalue='RANGE',value=1, $
                title='BIN: ',xsize=13,/return_events,/float)
...
;
	...
	info = {message:message,ns:ns,pixid:pixid,xtitle:xtitle, $
		...
		ymin_base:ymin_base,xmax_base:xmax_base,ymax_base:ymax_base, binsize_base:binsize_base, $
