; $Id: int_tabulated.pro,v 1.16 2000/01/21 00:29:12 scottm Exp $
;
...
FUNCTION Int_Tabulated_Var, X, F, Double = Double, Sort = Sort
  ...
  ;Compute the integral using the 5-point Newton-Cotes formula.
    ...
    if Double eq 0 then $
      RETURN, FLOAT(TOTAL(2.0 * h^2.0 * (7.0 * (z[ii-4] + z[ii]) + $
                    32.0 * (z[ii-3] + z[ii-1]) + 12.0 * z[ii-2]) / 45.0)) $
    else $
      RETURN, TOTAL(2D * h^2.0 * (7D * (z[ii-4] + z[ii]) + $
