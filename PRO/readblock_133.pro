        ...
        IF (!err NE -1) THEN BEGIN 
          ...
          FOR j = 1, map[i].border+1 DO BEGIN 
            map[i].bkg[j-1] = fxpar(btheader, 'bkg' + $
                                    strtrim(j, 2))
            map[i].bkgsd[j-1] = fxpar(btheader, 'bkgsd' + $
                                      strtrim(j, 2))
          ENDFOR 
          ...
          FOR j = 1, map[i].ngauss DO BEGIN 
            map[i].mean[j-1] = fxpar(btheader, 'mean' + $
                                     strtrim(j, 2))
            map[i].meansd[j-1] = fxpar(btheader, 'meansd' + $
                                       strtrim(j, 2))
            map[i].width[j-1] = fxpar(btheader, 'width' + $
                                      strtrim(j, 2))
            map[i].widthsd[j-1] = fxpar(btheader, 'widthsd' + $
                                        strtrim(j, 2))
            map[i].area[j-1] = fxpar(btheader, 'area' + $
                                     strtrim(j, 2))
            map[i].areasd[j-1] = fxpar(btheader, 'areasd' + $
                                       strtrim(j, 2))
          endfor 
        endif
      ENDIF  
