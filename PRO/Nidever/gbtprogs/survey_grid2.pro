...
fits_read,'../data/combine_final_coldens.fits',col,hd
fits_arrays,hd,gl,gb

gd = where(str.par[1] lt -100,ngd)
area = garea(str.par)
hess,str[gd].glon,str[gd].glat,area[gd],im,dx=0.5,dy=0.5,xr=[128,175],yr=[20,55],/total,xarr=xarr,yarr=yarr,/noplot
;im2 = gsmooth(im,2)

logcol = alog10(col>1)
bd = where(col eq 0,nbd)
logcol[bd] = -1
displayc,logcol,gl,gb,min=18,max=21,/xflip,maskv=-1,maskc=255
levels=findgen(20)*10.0+2.0
contour,im,xarr,yarr,levels=levels,/over,thick=2,color=250 ;0 ;255

;;ghess,str,'glon','glat',dx=0.5,dy=0.5,xr=[110,170],yr=[10,55],$
;ghess,str,'glon','glat',dx=0.5,dy=0.5,xr=[132,175],yr=[30,55],$
...
glcen7 = 140.9  ;155.7
gbcen7 = 47.0   ;46.2
mkhdr,hd7,fltarr(nx,ny)
sxaddpar,hd7,'CTYPE1','GLON-TAN'
sxaddpar,hd7,'CRPIX1',nx/2
sxaddpar,hd7,'CRVAL1',glcen7
sxaddpar,hd7,'CDELT1',3.5/60.
sxaddpar,hd7,'CTYPE2','GLAT-TAN'
sxaddpar,hd7,'CRPIX2',ny/2
sxaddpar,hd7,'CRVAL2',gbcen7
sxaddpar,hd7,'CDELT2',3.5/60.
x = findgen(nx)#replicate(1,ny)
y = replicate(1,nx)#findgen(ny)
xyad,hd7,x,y,glon7,glat7
oplot,glon7,glat7,ps=3,co=150

; Balance positions, along center, every 10 rows
xx = fltarr(ny/10)+nx/2
yy = findgen(ny/10)*10
xyad,hd7,xx,yy,gl,gb
print,'HVCAGrid7         '+strtrim(glcen7,2)+'     '+strtrim(gbcen7,2)+'  0.0'
printline,'Grid7Balance'+strtrim(indgen(13)+1,2)+'     '+strtrim(gl,2)+'     '+strtrim(gb,2)+'  0.0'
...
glcen6 = 146.4  ;155.7
gbcen6 = 43.8   ;46.2
mkhdr,hd6,fltarr(nx,ny)
sxaddpar,hd6,'CTYPE1','GLON-TAN'
sxaddpar,hd6,'CRPIX1',nx/2
sxaddpar,hd6,'CRVAL1',glcen6
sxaddpar,hd6,'CDELT1',3.5/60.
sxaddpar,hd6,'CTYPE2','GLAT-TAN'
sxaddpar,hd6,'CRPIX2',ny/2
sxaddpar,hd6,'CRVAL2',gbcen6
sxaddpar,hd6,'CDELT2',3.5/60.
x = findgen(nx)#replicate(1,ny)
y = replicate(1,nx)#findgen(ny)
xyad,hd6,x,y,glon6,glat6
oplot,glon6,glat6,ps=3,co=100

; grid 6, 26x30, ~1 hour
; grid 7, 155x80, 14.3 hours
; with 25 min. overhead per 3 hr. session that's a total of 17.4 hours
...
; overplot the LAB contours again
contour,im,xarr,yarr,levels=levels,/over,thick=2,color=250 ;0 ;255
