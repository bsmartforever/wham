...
SXADDPAR,head,'CRVAL1',ra_ref
SXADDPAR,head,'CTYPE1','RA'
SXADDPAR,head,'CDELT2',step
SXADDPAR,head,'CRPIX2',yref+1L
SXADDPAR,head,'CRVAL2',dec_ref
SXADDPAR,head,'CTYPE2','DEC'
...
For i=0,nfiles-1 do begin
  ...
  ; flip RA around
  bd = where(final.ra lt 180.,nbd)
  final[bd].ra = final[bd].ra+360

  ; Convert from RA/DEC to new X/Y
  ADXY,head,final.ra,final.dec,xnew,ynew
  ...
  For j=0,nchan-1 do begin
    ...
    grid = TRIGRID(xnew, ynew, final.comb_corr[j], tr, XOUT = xout, YOUT = yout, missing=0.0)

    ;temprms = rmsmap[xmin:xmax,ymin:ymax]
    ;gdgrid = where(rmsmap ge rms and finite(grid) eq 1,ngdgrid)
...
ps_open,'gbt_mstip_all3',/color,thick=4,/encap
...
ps_open,'gbt_mstip_new3a',/color,thick=4,/encap
device,/inches,xsize=12.3,ysize=12
...
ps_close
ps2gif,'gbt_mstip_new3a.eps',/eps
