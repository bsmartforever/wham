...
;  =perc      Calculate the Nth percentile.
...
undefine,xbin,ybin,gdind,xmnbin
...
if keyword_set(meanset) then oper=1  ; Mean, this is redundant
if keyword_set(med) then oper=2   ; Median
if keyword_set(std) then oper=3   ; Standard Deviation
if keyword_set(madset) then oper=4   ; MAD
if keyword_set(perc) then oper=5  ; Percentile
if keyword_set(hist) then oper=6  ; Histogram, i.e. number of points
if keyword_set(sum) then oper=7  ; Sum of points
if keyword_set(mode) then oper=8  ; Mode
