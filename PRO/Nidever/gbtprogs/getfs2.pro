...
; @keyword ifnum {in}{optional}{type=integer} IF number, default is 0.
; @keyword intnum {in}{optional}{type=integer} integration number,
; default is all integrations.
; @keyword plnum {in}{optional}{type=integer} polarization index (0 or
; 1), default is 0.
; @keyword fdnum {in}{optional}{type=integer} feed number, default is
; 0.
...
;
; @version $Id: getfs.pro,v 1.51 2008/02/12 22:28:42 bgarwood Exp $
;-
pro getfs2,scan,ifnum=ifnum,intnum=intnum,plnum=plnum,fdnum=fdnum,sampler=sampler,tsys=tsys,tau=tau,$
          ap_eff=ap_eff,smthoff=smthoff,units=units,nofold=nofold,eqweight=eqweight,$
          tcal=tcal,quiet=quiet,keepints=keepints,useflag=useflag,skipflag=skipflag,$
          instance=instance,file=file,timestamp=timestamp,status=status,rfi=rfi
    compile_opt idl2
    ...
    expectedCount = (n_elements(intnum) eq 1) ? 1 : info.n_integrations
    ...
    for n_int = 0,(expectedCount-1) do begin
        ...
        dofreqswitch2,data[sigwcal[n_int]],data[sig[n_int]],data[refwcal[n_int]],data[ref[n_int]],smthoff,$
                     tsys=tsys,tau=tau,tcal=tcal,sigResult=sigResult,refResult=refResult,rfi=rfi
        ...
        if thisnofold then begin
            ...
            folded = dcfold(sigResult,refResult)
