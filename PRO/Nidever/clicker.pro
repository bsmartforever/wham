...
;xr = [pos(0),pos(2)]
;yr = [pos(1),pos(3)]
xr = !x.crange    ; The min/max value of last plot
yr = !y.crange
...
if not keyword_set(noprint) then print,'Click outside the box OR hit the middle mouse button to stop selecting'

cursor,curx,cury
if keyword_set(overplot) then oplot,[curx],[cury],ps=8

while (curx le max(xr) and curx ge min(xr) and cury le max(yr) and cury ge min(yr)) and $
      !mouse.button ne 2 do begin
