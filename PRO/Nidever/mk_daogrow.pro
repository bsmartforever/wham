pro mk_daogrow,dir,file,silent=silent
...
files = file_search(dir,'*_*a.fits')
...
gd = where(stregex(files,'Raw',/boolean,/fold_case) eq 0 and $
           ...
           stregex(files,'zero',/boolean,/fold_case) eq 0 and $
           stregex(files,'SA110',/boolean,/fold_case) eq 0 and $
           stregex(files,'SA98',/boolean,/fold_case) eq 0 and $
           stregex(files,'NGC3680',/boolean,/fold_case) eq 0,ngd)
files = files(gd)
nfiles = n_elements(files)
...
for i=0,nfiles-1 do begin
  ...
  printf,unit,format=format,'',name,filt,uthr,utmin,am,exp
...
gd = where(stregex(apfiles,'Raw',/boolean,/fold_case) eq 0 and $
           ...
           stregex(apfiles,'zero',/boolean,/fold_case) eq 0 and $
           stregex(apfiles,'SA110',/boolean,/fold_case) eq 0 and $
           stregex(apfiles,'SA98',/boolean,/fold_case) eq 0 and $
           stregex(apfiles,'NGC3680',/boolean,/fold_case) eq 0,ngd)
apfiles = apfiles(gd)
napfiles = n_elements(apfiles)
