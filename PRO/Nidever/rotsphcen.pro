...
if n_elements(lon) eq 0 or n_elements(lat) eq 0 or n_elements(clon) eq 0 or $
  ...
  print,'Syntax - rotsphcen,lon,lat,clon,clat,nlat,nlon,polar=polar,gnomic=gnomic,stp=stp,'
...
IF not keyword_set(polar) and not keyword_set(gnomic) then begin
  ...
  if not keyword_set(reverse) then begin
    ...
    ROTSPH,lon,lat,np_lon,np_lat,nlon,nlat

  ; REVERSE
  endif else begin

    ROTSPH,lon,lat,np_lon,np_lat,nlon,nlat,/reverse
...
ENDIF ELSE BEGIN
  ...
  if not keyword_set(reverse) then begin
    ...
    ;  the Npole
    ROTSPH,lon,lat,clon[0],clat[0],phi,theta
  ...
  ;-----------------
    ...
    ;Run rotsph.pro and specify the center of the field (the origin) as the
    ;  the Npole
    ROTSPH,phi,theta,clon[0],clat[0],nlon,nlat,/reverse
