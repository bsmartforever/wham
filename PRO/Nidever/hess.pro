...
;   zz       Array of z-values.  Required for /total,/avg,/mean,/std
...
for i=0.,ng-1. do begin
  xind = floor((x(i)-xmin)/dx)
  yind = floor((y(i)-ymin)/dy)
  ims(xind,yind) = ims(xind,yind) + (z(i)-ima(xind,yind))^2.
endfor
...
pro hess_max,im,x,y,z,xmin,ymin,dx,dy
...
pro hess_perc,im,x,y,z,xmin,ymin,dx,dy,nx,ny,perc=perc
...
for i=1.,nbins-1. do binind(i) =  binind(i-1)+binpop1d(i-1)
...
if (n_params() eq 0) or (nxarr eq 0) or (nyarr eq 0) then begin
  ...
  print,'             std=std,mean=mean,colnorm=colnorm,'
...
; These sub-programs are at the bottom of this file
...
if keyword_set(maximum) then $
  hess_max,im,x,y,z,xmin,ymin,dx,dy
...
if keyword_set(perc) then $
  hess_perc,im,x,y,z,xmin,ymin,dx,dy,nx,ny,perc=perc
