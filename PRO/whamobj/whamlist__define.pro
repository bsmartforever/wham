...
function WHAMList::Map, vmin, vmax, zmin = zmin, zmax = zmax
  ...
  minlon = reduceto180(floor(min(lon) - padding) > 0)
  maxlon = reduceto180(ceil(max(lon) + padding) < 360)
...
;  ENDIF 
  ...
  m = map('Hammer', label_position = 0, font_size = 12, $
            limit = [minlat, minlon, maxlat, maxlon])
;    latitude_min = minlat, latitude_max = maxlat, $
;    longitude_min = minlon, longitude_max = maxlon)
...
;        fill_color = reform(colors[data_scaled[i], *])  )
  ...
  for i = 0, N_ELEMENTS(data_scaled)-1 do $
    points.add, ELLIPSE(lon[i], lat[i], $
      minor = 0.5, major = 0.5/cos(lat[i] * !dtor), /data, $
      linestyle = ' ', fill_color = reform(colors[data_scaled[i], *]))
...
;   
;  xy = map_proj_forward(lon, lat, map_structure = m.map_structure)
;  p = plot(xy, vert_colors = data_scaled, rgb_table = 3, $
...
pro WHAMList::GetProperty, name = name, $
  ...
  if ARG_PRESENT(int) then begin
    int = fltarr(self.count())
    foreach e, self, i do int[i] = e.int
  endif
