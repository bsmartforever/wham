pro multiplot_hi_ha,hi_arr,ha_arr,$
    vframe=vframe,mR=mR,adu=adu,$
    ygap=ygap,xgap=xgap,symsize=symsize,$
    device=device,ps=ps,filename=filename,$
    landscape=landscape,portrait=portrait,$
    xsize=xsize,ysize=ysize,$
    hi_factor=hi_factor,ha_factor=ha_factor,$
    bad_range1=bad_range1,bad_range2=bad_range2,$
    bad_color=bad_color,$
    hi_bin=hi_bin,ha_bin=ha_bin,$
    labels=labels,label_factor=label_factor,position=position,$
    left=left,right=right,top=top,bottom=bottom,$
    text=text,tcolor=tcolor,$
    xrange=xrange,hi_yrange=hi_yrange,ha_yrange=ha_yrange,yminimum=yminimum,ymaximum=ymaximum,$
    ...
    ymultiple=ymultiple,$
    ha_width=ha_width,ha_area=ha_area,ha_center=ha_center,$
    hi_width=hi_width,hi_area=hi_area,hi_center=hi_center

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;+
;
; Purpose: To allow users to flexibly plot multiple spectra on one plot. 
;          Allows users to plot HI column density and H-alpha Intensity 
;
;  hi_arr, ha_arr - String arrays containing the name of the spectra structure. 
;               Expected spectra tag names include vel, wave, flux, norm, and column. User must define hi_arr. If 
...
;               hi_arr, ha_arr, str3_arr, and str4_arr arrays.
...
;  position   - Position integer array of size hi_arr that specifies the location of the labels, 
...
;               of elements in hi_arr, then the unspecified panels will be labeled in the top right corner.
...
;                Only highlights spectra passed by hi_arr.
...
;
;
; e.g., hi_arr=['CAL_F_ALII1670IM']
;       multiplot_spectra,hi_arr 
; e.g., multiplot_spectra,[hi_arr,hi_arr,hi_arr,hi_arr,hi_arr,hi_arr,hi_arr,hi_arr],charsize=1.1,dim=[4,2],ygap=0.02,xgap=.01
;
...
;m=number of rows
;n=number of columns
   n_total=n_elements(hi_arr)
   m_total=2
if n_elements(hi_arr) ne n_elements(ha_arr) then begin
   print,''
   print,'*** Must have an equal number of HI and Ha ***'
   print,''
   return
endif
...
if (keyword_set(all_res_factor)) AND (NOT keyword_set(res_factor1)) AND (NOT keyword_set(res_factor2)) then begin
...
if (keyword_set(all_bin_factor)) AND (NOT keyword_set(bin_factor1)) then begin
...
variable='DATA'
if (NOT keyword_set(hi_factor)) then hi_factor=18. 
ytitle_hi='N!DHI!N (10!U'+strcompress(string(fix(hi_factor)),/re)+'!N cm!U-2!N)'

if keyword_set(mR) then ytitle_ha='I!DH!7a!3!N (mR)' else ytitle_ha='I!DH!7a!3!N (R)'
...
if (keyword_set(xtitle)) and (n_elements(xtitle) ne n_total*m_total) then begin
  ...
  grand_xtitle=' ' 
  xtitle=xtitle
endif else if (NOT keyword_set(xtitle)) AND (n_elements(xtitle) ne n_total*m_total) then begin
...
xtag='vel'
if (NOT validate_numeric(vframe)) then vframe=[0.0]
...
if (NOT keyword_set(xrange)) then begin
  if (strmatch(hi_arr[0],'*BLANK*',/fold_case)) then begin
    print,''
    print,'*** Because the first element is BLANK, you must specify the xrange ***'
    print,''
    return
  endif
    xtag_loc=where(strcmp(tag_names((SCOPE_VARFETCH(hi_arr[0], /ENTER,level=1))),xtag,/fold_case) eq 1)
    x=(SCOPE_VARFETCH(hi_arr[0], /ENTER,level=1)).(xtag_loc)
...
if keyword_set(labels) AND n_elements(position) ne n_total*m_total then begin
   print,'*** Number of elements in position ne to number of elements in hi_arr. ***'
...
!p.position=[0.125,0.2,0.95,0.95]

if (NOT keyword_set(ygap)) then ygap=0.0
if (NOT keyword_set(xgap)) then xgap=0.0
if (NOT keyword_set(hi_yrange)) AND (NOT keyword_set(ha_yrange)) AND (n_elements(xgap) eq 0) then xgap = 0.035*charsize
...
m=0
     ...
     if (NOT strmatch(hi_arr[q],'*BLANK*',/fold_case)) then begin
       xtag_loc=where(strcmp(tag_names((SCOPE_VARFETCH(hi_arr[q], /ENTER,level=1))),xtag,/fold_case) eq 1)
       ytag_loc=where(strcmp(tag_names(SCOPE_VARFETCH(hi_arr[m], /ENTER,level=1)),VARIABLE,/fold_case) eq 1)    
       ytag_err_loc=where(strcmp(tag_names(SCOPE_VARFETCH(hi_arr[q], /ENTER,level=1)),'VAR',/fold_case) eq 1) 
     endif  
       ...
       !p.position=[ip[2]-(ip[2]-ip[0])/n_total*(n_total-n)+xgap/2.0,$
        ...
        if (n_elements(SCOPE_VARFETCH(hi_arr[q], /ENTER,level=1)) eq 0) AND (NOT strmatch(hi_arr[q],'*BLANK*',/fold_case)) then begin
           print,' '
           print,'*** Variable name ',hi_arr[q],' does not exist!!! ***'
           print,' '
           return
        endif

          if (NOT strmatch(hi_arr[q],'*BLANK*',/fold_case)) then begin
            x1=(SCOPE_VARFETCH(hi_arr[q], /ENTER,level=1)).(xtag_loc)
            hi_str=(SCOPE_VARFETCH(hi_arr[q], /ENTER,level=1))
            if strmatch(variable,'data',/fold_case) then x1=hi_str.(xtag_loc)
            y1=hi_str.(ytag_loc)/10^hi_factor*!k2cm2
            y1_err=sqrt(hi_str.(ytag_err_loc))/10^hi_factor*!k2cm2
          endif else begin 
            x1=[0,0]
            y1=[0,0]
          endelse  

      if ((keyword_set(yminimum)) AND (n_elements(yminimum) ne m_total*n_total)) OR $
         ((keyword_set(ymaximum)) AND (n_elements(ymaximum) ne m_total*n_total)) then begin
         print,' '
         print,'*** n_elements of ymin & ymax must be equal to n_elements of hi_arr ***'
         ...
         hi_yrange=[min([yminimum[q],ymaximum[q]]),max([yminimum[q],ymaximum[q]])]
      endif else if (keyword_set(yminimum)) AND (keyword_set(yaxis_factor)) then begin
         hi_yrange=axis_factor_range(yaxis_factor[0],yaxis_factor[1:2],min=yminimum[q])
      endif else if (keyword_set(ymaximum)) AND (keyword_set(yaxis_factor)) then begin
         hi_yrange=axis_factor_range(yaxis_factor[0],yaxis_factor[1:2],max=ymaximum[q])
      endif 

      if keyword_set(hi_yrange) then yrange_user=hi_yrange
      ...
      if ((NOT keyword_set(yrange_user)) AND (NOT keyword_set(yminimum)) AND (NOT keyword_set(ymaximum))) OR $
         ...
         ((NOT keyword_set(yrange_user)) AND (NOT keyword_set(ymaximum)) AND (NOT keyword_set(yaxis_factor))) then $          
           hi_yrange=min_max_multi(x1,y1,x2,y2,x3,y3,x4,y4,range=xrange)

      xticks=xticks_original
      yticks=yticks_original

      padding=abs(hi_yrange[1]-hi_yrange[0])*.1
      if keyword_set(yticks) then tickv,hi_yrange,yticks,ytickname,ytickv,padding=padding,multiple=multiple,/zero $
      else begin
        ytickv=0
        ytickname=''
      endelse
      ...
      if (keyword_set(hi_bin)) then begin
        length=fix(n_elements(x1)/hi_bin)
        x1=frebin(x1,length)
        y1=frebin(y1,length)
        y1_err=frebin_err(y1_err,length)
      endif

       plot,x1-vframe[0],y1,psym=10,$
            ytitle=' ',xtitle=xtitle[m],$
            charsize=charsize*0.75,charthick=charthick,thick=thick,$
            xrange=xrange,yrange=hi_yrange,$
       ...
       if n_elements(line) ne 0 then begin
          ...
          for r=0, n_elements(line)-1 do begin
            oplot,line[r]*[1.,1.],!y.crange,linestyle=2,color=fsc_color('black')
          endfor
       endif
       ...
       gauss_total=x1*0.
      if keyword_set(hi_area) then begin
        for z=0,n_elements(hi_width)-1 do begin
          if hi_width[z] ne 0 then begin
              sigma=hi_width[z]/(2.*sqrt(2.*alog(2)))
              height=hi_area[z]/(sigma*sqrt(2.0*!pi))*(10^hi_factor)^(-1.)
              gauss=gaussian(x1-vframe[0],[height,hi_center[z],sigma])
              vgood=where((x1 ge hi_center[z]-3.0*sigma) AND (x1 le hi_center[z]+3.0*sigma))
              gauss_total[vgood]=gauss_total[vgood]+gauss[vgood]
              oplot,x1[vgood]-vframe[0],gauss[vgood],color=fsc_color('blk5'),thick=!p.thick+5
              oplot,x1[vgood]-vframe[0],gauss_total[vgood],color=fsc_color('blk5'),thick=!p.thick+5
          endif
        endfor
      endif

        oploterror,x1-vframe[0],y1,$
          x1*0.0,y1_err,$
          psym=10,$ ;symcat(16),symsize=symsize,$
          /nohat,errcolor=fsc_color('blk6'),$
           errthick=!p.thick+2

        oplot,!x.crange,[0,0],linestyle=1,thick=thick
       ...
       if n_elements(bad_range1) ne 0 then begin
          ...
          if count ne 0 then begin
            ...
            bad_index=where((x1_big-vframe[0] ge min(bad_range1[tmp,1:2])) AND (x1_big-vframe[0] le max(bad_range1[tmp,1:2])),count)
              if count ne 0 then oplot,x1_big[bad_index]-vframe[0],y1_big[bad_index],psym=10,color=bad_color,thick=thick+2
          endif
       endif
            ...
            if keyword_set(fill) then begin
              for f=0,n_elements(fill[0,*])-1 do begin
                  if tagfill[f,q] eq f+1 then begin
                      ...
                      if num_good ne 0 then plotcolorfill,x1[good_fill]-vframe[0],y1[good_fill],/noerase,bottom=bottom,color=fsc_color('blk4') 
                             oplot,x1-vframe[0],y1,psym=10, color=color[0],thick=thick
                  endif
              endfor
            endif 
               ...
               if count_arrows ne 0 then begin
                  for f=0, count_arrows-1 do begin
                    ...
                    if arrows[0,good_arrows[f]] gt 0 then begin
                       min_height=y1[xloc]+(!y.crange[1]-!y.crange[0])*0.150
                       max_height=y1[xloc]+(!y.crange[1]-!y.crange[0])*0.275
                       label_height=y1[xloc]+(!y.crange[1]-!y.crange[0])*0.3
                       ...
                       min_height=y1[xloc]-(!y.crange[1]-!y.crange[0])*0.140
                       max_height=y1[xloc]-(!y.crange[1]-!y.crange[0])*0.275
                       label_height=y1[xloc]-(!y.crange[1]-!y.crange[0])*0.4
                       arrow,arrows[1,good_arrows[f]],max_height,arrows[1,good_arrows[f]],min_height,/data,thick=!p.thick,hsize=(!D.X_SIZE / 100.) 
                       xyouts,arrows[1,good_arrows[f]],label_height,strcompress(string(fix(good_arrows[f]+1)),/re),align=0.5,/data,$
                        charsize=charsize*0.75,charthick=charthick
                    endelse
       ...
       if keyword_set(labels) or (num_text ne 0) then begin
           ...
           if (NOT strmatch(hi_arr[q],'*BLANK*',/fold_case)) then begin
             if tag_loc((SCOPE_VARFETCH(hi_arr[q], /ENTER,level=1)),'ion') eq 1 then begin
              if ((SCOPE_VARFETCH(hi_arr[q], /ENTER,level=1)).ion eq 'HI')  then $
                 label1=(SCOPE_VARFETCH(hi_arr[q], /ENTER,level=1)).ion else $
                 label1=(SCOPE_VARFETCH(hi_arr[q], /ENTER,level=1)).ion+' '+lambda $
                      +strcompress(string((SCOPE_VARFETCH(hi_arr[q], /ENTER,level=1)).wavc,$
                      format='(I4)'),/re)
                ...
                if keyword_set(ha_str) then begin    
                  if (SCOPE_VARFETCH(hi_arr[q], /ENTER,level=1)).ion eq 'HI' then $
                    label2=(SCOPE_VARFETCH(hi_arr[q], /ENTER,level=1)).ion else if (NOT strmatch(ha_arr[q],'*BLANK*',/fold_case)) then $
                    label2=(SCOPE_VARFETCH(ha_arr[q], /ENTER,level=1)).ion+' '+lambda $
                      +strcompress(string((SCOPE_VARFETCH(hi_arr[q], /ENTER,level=1)).wavc,$
                      format='(I4)'),/re) else label2=''
      
                  if (label1 eq label2) AND keyword_set(labels) then xyouts,pos[0],pos[1],label1,align=0.5,/normal,$
                     charsize=charsize*label_factor,charthick=charthick,color=tcolor
                endif
              endif
             endif
         endif
               ...
               if keyword_set(text) then begin
                   ...
                   if (NOT keyword_set(label_factor)) then label_factor=1.5
                   pos=[(!p.position[2]-!p.position[0])*7./8.0+!p.position[0],$
                        (!p.position[3]-!p.position[1])*7./8.0+!p.position[1]]   
                  ;right
                  if (position[q] eq 0) or (position[q] eq 1) then pos[0]=(!p.position[2]-!p.position[0])*7.0/8.0+!p.position[0]
                  ;left
                  if (position[q] eq 2) or (position[q] eq 3) then pos[0]=(!p.position[2]-!p.position[0])*1.25/8.0+!p.position[0]
       ...
       q=q+1
        ...
        grand_ytitle=ytitle_hi
        if device eq 'X' then xpos = 0.04 else xpos = 0.03+n*0.01
        xyouts,xpos,0.75,grand_ytitle,/normal,orientation=90,align=0.5,charsize=charsize,charthick=charthick

q=0
for n=0,n_total-1 do begin 
m=1

     if (NOT strmatch(ha_arr[q],'*BLANK*',/fold_case)) then begin
       xtag_loc=where(strcmp(tag_names((SCOPE_VARFETCH(ha_arr[q], /ENTER,level=1))),xtag,/fold_case) eq 1)
       ytag_loc=where(strcmp(tag_names(SCOPE_VARFETCH(ha_arr[q], /ENTER,level=1)),VARIABLE,/fold_case) eq 1)    
       ytag_err_loc=where(strcmp(tag_names(SCOPE_VARFETCH(ha_arr[q], /ENTER,level=1)),'VAR',/fold_case) eq 1)  
     endif  
       ...
       !p.position=[ip[2]-(ip[2]-ip[0])/n_total*(n_total-n)+xgap/2.0,$
          ...
          if n_elements(ha_arr) eq n_total*(m_total-1) then begin
             if n_elements(SCOPE_VARFETCH(ha_arr[q], /ENTER,level=1)) eq 0 AND (NOT strmatch(ha_arr[q],'*BLANK*',/fold_case)) then begin
                print,' '
                print,'*** Variable name ',ha_arr[q],' does not exist!!! ***'
                print,' '
                return
             endif
             if (NOT strmatch(ha_arr[q],'*BLANK*',/fold_case)) then begin
              x1=(SCOPE_VARFETCH(ha_arr[q], /ENTER,level=1)).(xtag_loc)
              ha_str=(SCOPE_VARFETCH(ha_arr[q], /ENTER,level=1))
              if strmatch(variable,'data',/fold_case) then x1=ha_str.(xtag_loc)
              if keyword_set(mR) then mR_factor=1e3 else mR_factor=1.
              if keyword_set(ADU) then ADU_factor=1./22.8 else ADU_factor=1.       
              if (NOT keyword_set(ha_factor)) then ha_factor=mR_factor*ADU_factor       
              y1=ha_str.(ytag_loc)*ha_factor
              y1_err=sqrt(ha_str.(ytag_err_loc))*ha_factor
            endif else begin 
              x1=[0,0]
              y1=[0,0]
            endelse  
      ...
      if ((keyword_set(ymin3um)) AND (n_elements(yminimum) ne m_total*n_total)) OR $
         ...
         (n_elements(yminimum) eq m_total*n_total) AND (n_elements(ymaximum) eq m_total*n_total) then begin
         ha_yrange=[min([yminimum[q+1],ymaximum[q+1]]),max([yminimum[q+1],ymaximum[q+1]])]*ha_factor
      endif else if (keyword_set(yminimum)) AND (keyword_set(yaxis_factor)) then begin
         ha_yrange=axis_factor_range(yaxis_factor[0],yaxis_factor[1:2],min=yminimum[q+1])*ha_factor
      endif else if (keyword_set(ymaximum)) AND (keyword_set(yaxis_factor)) then begin
         ha_yrange=axis_factor_range(yaxis_factor[0],yaxis_factor[1:2],max=ymaximum[q+1])*ha_factor
      endif 

      undefine,yrange_user
      if keyword_set(ha_yrange) then yrange_user=ha_yrange

      if ((NOT keyword_set(yrange_user)) AND (NOT keyword_set(yminimum)) AND (NOT keyword_set(ymaximum))) OR $
         ((NOT keyword_set(yrange_user)) AND (NOT keyword_set(yminimum)) AND (NOT keyword_set(yaxis_factor))) OR $
         ((NOT keyword_set(yrange_user)) AND (NOT keyword_set(ymaximum)) AND (NOT keyword_set(yaxis_factor))) then $          
           ha_yrange=min_max_multi(x1,y1,x2,y2,x3,y3,x4,y4,range=xrange)

      xticks=xticks_original
      yticks=yticks_original

      padding=abs(ha_yrange[1]-ha_yrange[0])*.1
      if keyword_set(yticks) then tickv,ha_yrange,yticks,ytickname,ytickv,padding=padding,multiple=multiple,/zero $
      else begin
        ytickv=0
        ytickname=''
      endelse
      ...
      if (keyword_set(ha_bin)) then begin
        length=fix(n_elements(x1)/ha_bin)
        x1=frebin(x1,length)
        y1=frebin(y1,length)
        y1_err=frebin_err(y1_err,length)
      endif
       ...
       plot,x1-vframe[0],y1,psym=10,$
            ytitle=' ',xtitle=xtitle[m],$
            charsize=charsize*0.75,charthick=charthick,thick=thick,$
            xrange=xrange,yrange=ha_yrange,$
            ystyle=1,/noerase,yticks=yticks,$
            xtickname=axvalue(xtickname,m_total-1,100,blank),$
            ytickname=axvalue(ytickname,0,n,blank),$
            ytickv=ytickv,xtickv=xtickv,yminor=yminor,$
            ...
            if keyword_set(ha_area) then begin
              if ha_width[q] ne 0 then begin
                  sigma=ha_width/(2.*sqrt(2.*alog(2)))
                  height=ha_area/(sigma*sqrt(2.0*!pi))
      
                  gauss=gaussian(x1-vframe[0],[height[q],ha_center[q],sigma[q]])
                  instrument_profile=wham_ip(x1-vframe[0])
                  wham_gauss=CONVOL(gauss,instrument_profile.ip, /CENTER, /EDGE_TRUNCATE)
      
                  vgood=where((x1 ge ha_center[q]-3.0*sigma[q]) AND (x1 le ha_center[q]+3.0*sigma[q]))
                  oplot,x1[vgood]-vframe[0],wham_gauss[vgood],color=fsc_color('blk5'),thick=!p.thick+5
              endif
            endif
       ...
       if keyword_set(labels) or (num_text ne 0) then begin
           ...
           if (NOT strmatch(hi_arr[q],'*BLANK*',/fold_case)) then begin
             if tag_loc((SCOPE_VARFETCH(hi_arr[q], /ENTER,level=1)),'ion') eq 1 then begin
              ...
              ;    if (NOT keyword_set(label_factor)) then label_factor=1.25
              ;    pos=[(!p.position[2]-!p.position[0])*6./8.0+!p.position[0],$
              ;         (!p.position[3]-!p.position[1])*6./8.0+!p.position[1]]   
              ;   ;right
              ;   if (position[q] eq 0) or (position[q] eq 1) then pos[0]=(!p.position[2]-!p.position[0])*7.0/8.0+!p.position[0]
       ...
       q=q+1
        ...
        grand_ytitle=ytitle_ha
        if device eq 'X' then xpos = 0.04 else xpos = 0.03+n*0.01
        xyouts,xpos,0.4,grand_ytitle,/normal,orientation=90,align=0.5,charsize=charsize,charthick=charthick

if n_elements(ytitle_hi) eq n_elements(hi_str) then !p.position=[0.15,0.10,0.9,0.95] $
   else !p.position=[0.15,0.10,0.95,0.95]

if device eq 'X' then ypos = 0.025 else ypos = 0.1-m*0.0001
...
if keyword_set(title) then $
  xyouts,(!p.position[3]-!p.position[1])/2.0+!p.position[1],!p.position[3]+0.015,title,$
    /normal,align=0.5,charsize=charsize,charthick=charthick
