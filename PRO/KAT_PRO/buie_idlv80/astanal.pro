...
pro astanal,fnfit,fncenter,NODISPLAY=nodisplay,SILENT=silent, $
   ...
   for i=0,nze-1 do begin
      ft=''
      if exists(path+effn[i]+'.fits') then ft='.fits'
      if exists(path+effn[i]+ft) then begin
         hdr=headfits(path+effn[i]+ft)
         parsekey,hdr,keylist,info
         filter[i] = info.filter
         jdmid[i]  = info.jd
      endif else begin
         print,self,'Warning!  File ',effn[i],' is in fitcoeff.dat and image file is missing.'
         filter[i] = 'skipit'
      endelse
   endfor

   z=where(filter ne 'skipit',count)
   if count eq 0 then begin
      print,self,'None of the image files are present.  Quitting.'
      return
   endif
   time=(jdmid - min(jdmid[z]))*24.0
   ...
   for i=0,nfils-1 do begin
      ...
      if fillist[i] eq 'skipit' then begin
         print,strn(nobs),' entries skipped for missing images.'
         bad[z]=1B
         continue
      endif
