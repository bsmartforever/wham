...
;----------------------------------------------------------------------------
pro mycont,x,y,xarray,yarray,store,coflag
...
;loadct,39
	...
	IF coflag NE 0 THEN BEGIN
	   ...
	   FOR k=0,n_store-1 DO BEGIN
		...
		OPLOT,[store(0,k),store(1,k)],[1.0,1.0]*0.9*!y.crange(1)
	   ENDFOR
	ENDIF
;
;Get input from user.  If right mouse button is pushed, the return.
;If middle button is pushed, then return. 
;
...
;Mark left of region.
;
	PRINT,'IMCONT(v5.2):: Limit 1:  ',xpos1,format='(A-35,f8.2)'
	OPLOT,[xpos1,xpos1],liney1
;
;Get and mark right of region.
;
	CURSOR,xpos2,ypos2,/DOWN	
	xpos2 = xpos2 < MAX(x)  &  !c = 0
  	PRINT,'IMCONT(v5.2):: Limit 2: ',xpos2,format='(A-35,f8.2)'
  	tmp=xpos1
	xpos1=min([xpos1,xpos2])
	xpos2=max([tmp,xpos2])
	OPLOT,[xpos2,xpos2],liney1
;
;Check to be sure range is not reversed.  If reversed, get again.
;
	IF xpos2 GT xpos1 THEN BEGIN
		OPLOT,[xpos1,xpos2],[1,1]*0.9*!y.crange(1)	
