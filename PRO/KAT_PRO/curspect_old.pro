PRO curspect, map, map2, map3, map4, map_scale = map_scale, $
              norm=norm,$
              intrange = intrange, hi=hi, ha=ha,$
              image = image, radec = radec, $
              ;hi1 = hi1, hi2 = hi2, $
...
if (keyword_set(lon) AND keyword_set(lat)) or (keyword_set(glon) AND keyword_set(glat)) or (keyword_set(mlon) AND keyword_set(mlat)) then begin
   ...
   if (keyword_set(mlon) AND keyword_set(mlat)) then begin
      user_lon=mlon
      lon_index=where(user_lon lt 0,count)
...
if keyword_set(filename) OR keyword_set(PS) then device='PS' else device='X'
  ...
  if ((NOT keyword_set(user_lon)) AND (NOT keyword_set(user_lat))) AND (NOT keyword_set(index)) then $
  ...
  WHILE NOT done DO BEGIN 
    ...
    IF NOT done THEN BEGIN
      ...
      if (NOT keyword_set(magellanic)) AND (NOT keyword_set(index)) then near = spectnear(map, lon, lat, rad1, ncnt) $
         else if (NOT keyword_set(index)) AND (keyword_set(magellanic)) then begin
              lon_index = where(map.mlon lt 0.0,count)
              if count ne 0 then map[lon_index].mlon=map[lon_index].mlon+360.
              near = spectnear(map, lon, lat, rad1, ncnt,/magellanic)
         endif
...
vpmin = max(map[0].vel) - 203
              velslice = where(map[0].vel GE vpmin, vcount)
...
endelse
      ...
      ENDIF ELSE BEGIN 
        ...
        ENDIF ELSE BEGIN 
          ave_spectrum=sparith(map[near],/ave)
          IF (n_elements(intrange) EQ 2) AND (NOT keyword_set(hi)) AND (NOT keyword_set(ha)) THEN $
            print, 'Intensity (' + strtrim(intrange[0], 2) + ' - ' + $
            strtrim(intrange[1], 2) + ') = ' + $
            strtrim(intspect((ave_spectrum), intrange[0], $
                             intrange[1]), 2)  $
          else IF (n_elements(intrange) EQ 2) AND (NOT keyword_set(hi)) THEN $
            print, 'Intensity (' + strtrim(intrange[0], 2) + ' - ' + $
            strtrim(intrange[1], 2) + ') = ' + $
            strtrim(intspect((ave_spectrum), intrange[0], $
                             intrange[1])/22.8, 2) +' R' $
          ELSE IF (n_elements(intrange) EQ 2) THEN $ 
            print, 'Column Density (' + strtrim(intrange[0], 2) + ' - ' + $
            strtrim(intrange[1], 2) + ') = ' + $
            strtrim(intspect(ave_spectrum, intrange[0], $
                             intrange[1])*1.823e18, 2) +' cm^2'
          ...
          IF over THEN BEGIN
            ...
            !y = ys1
            IF NOT keyword_set(hi1) THEN begin 
              oplot, ((ave_spectrum).vel)[velslice], ((ave_spectrum).data)[velslice],$
               nsum = nsum, color=fsc_color(arr_color[0])
            endif ELSE begin
              oplot, (ave_spectrum).vel, (ave_spectrum).data, nsum = nsum,$
               color=fsc_color(arr_color[0])
            endelse
          ENDIF ELSE BEGIN 
          if (NOT keyword_set(magellanic)) then $
            ...
            l=((map[near[0]])).mlon
            if l gt 0 then l=l-360.
            b=((map[near[0]])).mlat
            title = 'Block ' + shname $
              + ' (mlon=' + strtrim(string(l,format='(f8.2)'), 2) $
              + ' mlat=' + strtrim(string(b,format='(f8.2)'), 2) + ')'
          endelse


            IF (NOT keyword_set(hi1)) AND (NOT keyword_set(norm)) THEN begin 
              vpmin = max((ave_spectrum).vel) - 203
              velslice = where((ave_spectrum).vel GE vpmin, vcount)

              plot, ((ave_spectrum).vel)[velslice], ((ave_spectrum).data)[velslice]*map_scale[0], $
                    title = title, xtitle='Velocity [km/s]',ytitle='Intensity', $
                    xstyle = 3, ystyle = 3, $
                    _extra = e, nsum = nsum,xrange=xrange,yrange=yrange
              oplot,!x.crange,[0,0],linestyle=1
            endif else IF (NOT keyword_set(hi1)) AND (keyword_set(norm)) THEN begin 
              vpmin = max((ave_spectrum).vel) - 203
              velslice = where((ave_spectrum).vel GE vpmin, vcount)
              plot, ((ave_spectrum).vel)[velslice], normal(((ave_spectrum).data)[velslice]), $
                    title = title, xtitle='Velocity [km/s]',ytitle='Intensity', $
                    xstyle = 3, ystyle = 3, $
                    _extra = e, nsum = nsum,xrange=xrange,yrange=[-0.25,1.25]
             oplot,!x.crange,[0,0],linestyle=1
            endif ELSE if (keyword_set(norm)) then begin
              plot, (ave_spectrum).vel, normal(ave_spectrum.data[velslice]), $
                    title = title, xtitle='Velocity [km/s]',ytitle='Intensity',$
                    xstyle = 3, ystyle = 3, $
                    _extra = e, nsum = nsum,color=color,xrange=xrange,yrange=[-0.25,1.25]
              oplot, (ave_spectrum).vel[velslice], normal((ave_spectrum.data)[velslice]), color=cgcolor(arr_color[0])     
             oplot,!x.crange,[0,0],linestyle=1,color=color
            endif ELSE begin
              plot, (ave_spectrum).vel, (ave_spectrum).data*map_scale[0], $
                    title = title, xtitle='Velocity [km/s]',ytitle='Intensity',$
                    xstyle = 3, ystyle = 3, $
                    _extra = e, nsum = nsum,color=color,xrange=xrange
              oplot, (ave_spectrum).vel, (ave_spectrum).data*map_scale[0], color=cgcolor(arr_color[0])     
             oplot,!x.crange,[0,0],linestyle=1,color=color
            endelse
          ...
          IF n_elements(map2) NE 0 THEN BEGIN
            ...
            ENDIF ELSE BEGIN 
              print, ncnt, ' pointing(s) in map #2 near, using element ' $
                + strtrim(near[0], 2) 
              ave_spectrum2=sparith(map2[near],/ave)

              IF (NOT keyword_set(hi2)) AND (NOT keyword_set(norm)) THEN begin 
                vpmin = max(ave_spectrum2.vel) - 203
                velslice = where(ave_spectrum2.vel GE vpmin, vcount)
                
                oplot, ave_spectrum2.vel[velslice], $
                  ave_spectrum2.data[velslice]*map_scale[1], $
                  color = cgcolor(arr_color[1]), nsum = nsum
              endif else IF (NOT keyword_set(hi2)) AND (keyword_set(norm)) THEN begin 
                vpmin = max(ave_spectrum2.vel) - 203
                velslice = where(ave_spectrum2.vel GE vpmin, vcount)
                
                oplot, ave_spectrum2.vel[velslice], $
                  normal(ave_spectrum2.data[velslice]), $
                  color = cgcolor(arr_color[1]), nsum = nsum
              endif else if (keyword_set(norm)) then begin 
                oplot, ave_spectrum2.vel[velslice], normal((ave_spectrum2.data)[velslice]), $
                  color = cgcolor(arr_color[1]), nsum = nsum
              endif else begin 
                oplot, ave_spectrum2.vel, ave_spectrum2.data*map_scale[1], $
                  color = cgcolor(arr_color[1]), nsum = nsum
              ENDELSE 
            endelse 
          endif 
          ...
          IF n_elements(map3) NE 0 THEN BEGIN
            ...
            IF ncnt EQ 0 THEN BEGIN
              print, 'No spectra from map #2 within ' + strtrim(rad2, 2) $
                     + ' degrees of click'
            ENDIF ELSE BEGIN 
            ave_spectrum3=sparith(map3[near],/ave)
              print, ncnt, ' pointing(s) in map #2 near, using element ' $
                + strtrim(near[0], 2) 

              IF (NOT keyword_set(hi2)) AND (NOT keyword_set(norm)) THEN begin 
                vpmin = max(ave_spectrum3.vel) - 203
                velslice = where(ave_spectrum3.vel GE vpmin, vcount)
              
                oplot, ave_spectrum3.vel[velslice], $
                  ave_spectrum3.data[velslice]*map_scale[2], $
                ...
                oplot, ave_spectrum3.vel[velslice], $
                  normal(ave_spectrum3.data[velslice]), $
                  color = cgcolor(arr_color[2]), nsum = nsum
              endif else if (keyword_set(norm)) then begin 
                oplot, ave_spectrum3.vel[velslice], normal(ave_spectrum3.data[velslice]), $
                  color = cgcolor(arr_color[2]), nsum = nsum
              endif else begin 
                oplot, ave_spectrum3.vel, ave_spectrum3.data*map_scale[2], $
                  color = cgcolor(arr_color[2]), nsum = nsum
              ENDELSE
            endelse 
          endif 
          ...
          IF n_elements(map4) NE 0 THEN BEGIN
            ...
            ENDIF ELSE BEGIN 
              print, ncnt, ' pointing(s) in map #2 near, using element ' $
                + strtrim(near[0], 2) 
              ave_spectrum4=sparith(map4[near],/ave)

              IF (NOT keyword_set(hi2)) AND (NOT keyword_set(norm)) THEN begin 
                vpmin = max(ave_spectrum4.vel) - 203
                velslice = where(ave_spectrum4.vel GE vpmin, vcount)
              
                oplot, ave_spectrum4.vel[velslice], $
                  ave_spectrum4.data[velslice]*map_scale[3], $
                ...
                oplot, ave_spectrum4.vel[velslice], $
                  (ave_spectrum4.data[velslice]), $
                  color = cgcolor(arr_color[3]), nsum = nsum
              endif else if (keyword_set(norm)) THEN begin 
                  oplot, ave_spectrum4.vel[velslice], normal(ave_spectrum4.data[velslice]), $
                  color = cgcolor(arr_color[3]), nsum = nsum
              ENDif else begin 
                  oplot, ave_spectrum4.vel, ave_spectrum4.data*map_scale[3], $
                  color = cgcolor(arr_color[3]), nsum = nsum
              ENDELSE 
            endelse 
          endif 
