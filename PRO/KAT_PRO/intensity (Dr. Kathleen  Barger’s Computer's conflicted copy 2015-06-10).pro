...
if keyword_set(click) then begin
   ...
   if keyword_set(Gauss) then begin
      ...
      for i=0,n_elements(fit_params)-1 do $
        ;GAUSS,fit.x,fit_params[i].center,fit_params[i].width,fit_params[i].height,Y
        oplot,fit.x,gaussian(fit.x,[fit_params[i].height,fit_params[i].center,$
          fit_params[i].width])+ycon[good],color=fsc_color('Dodger Blue'),linestyle=1
