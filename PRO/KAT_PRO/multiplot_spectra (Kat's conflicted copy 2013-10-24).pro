pro multiplot_spectra,str1_arr,str2_arr,str3_arr,str4_arr,$
    ...
    color=color,$
    line=line,fill=fill,tagfill=tagfill,$
    arrows=arrows,xticklen=xticklen,yticklen=yticklen,xtickthick=xtickthick,$
    ymultiple=ymultiple
...
;               If apparent is set, then ytitle='Apparent Column Density [cm!U-2!N/(km s!U-2!N)]'
;               If sub is set, then ytitle='Residuals of Apparent Column Density [cm!U-2!N/(km s!U-2!N)]'
...
if keyword_set(norm) then begin 
   ...
   variable='COLUMN' 
   if NOT keyword_set(ytitle) then begin
      grand_ytitle='Apparent Column Density (cm!U-2!N/(km s!U-2!N))'
...
if n_total*m_total eq n_elements(xtitle) then begin
  grand_xtitle=' ' 
  xtitle=xtitle
   ...
   xtitle=replicate(' ',n_total*m_total)
endif else if (keyword_set(xtitle)) and (n_elements(xtitle) ne n_total*m_total) OR $
   (n_total eq 1.0) then begin
...
if strmatch(device,'ps',/fold_case) then begin
  ...
  ;Use the following below instead to define the size of the plot. 
  ;This will enable a default calculation of the x and y offsets 
  ;for a given x and ysize. 
  ;Can also set landscape. Portait is default.
  ;keywords = PSConfig(Cancel=cancelled,/nogui,xsize=5,ysize=5,Encapsulated=1,Filename='tree.eps',FontType=-1)
...
if n_elements(ytitle) eq n_elements(str1) then !p.position=[0.125,0.1,0.9,0.95] $
   else !p.position=[0.125,0.1,0.95,0.95]
...
for n=0,n_total-1 do begin 
    for m=0, m_total-1 do begin 
     ...
     if (NOT strmatch(str1_arr[q],'BLANK',/fold_case)) then begin
       ...
       tag_loc=where(strcmp(tag_names(SCOPE_VARFETCH(str1_arr[m+n], /ENTER,level=1)),VARIABLE,/fold_case) eq 1)    
     endif  
       ...
       if (NOT keyword_set(sub)) then begin
          if (bin_factor1[q] ne 1.) OR (res_factor1[q] ne 1.) then begin
            str1=rebin_reduce_res((SCOPE_VARFETCH(str1_arr[q], /ENTER,level=1)),bin_factor=bin_factor1[q],res_factor=res_factor1[q]) 
            x1=str1.(xtag_loc)
            y1=str1.(tag_loc)
          endif else if (NOT strmatch(str1_arr[q],'BLANK',/fold_case)) then begin
            str1=(SCOPE_VARFETCH(str1_arr[q], /ENTER,level=1))
            x1=str1.(xtag_loc)
            y1=str1.(tag_loc)
          endif else begin 
            x1=[0,0]
          ...
          if n_elements(str2_arr) eq n_total*m_total then begin
             if n_elements(SCOPE_VARFETCH(str2_arr[q], /ENTER,level=1)) eq 0 then begin
                print,' '
                print,'*** Variable name ',str2_arr[q],' does not exist!!! ***'
                print,' '
                return
             endif
             if (bin_factor2[q] ne 1.) OR (res_factor2[q] ne 1.) then begin
              str2=rebin_reduce_res((SCOPE_VARFETCH(str2_arr[q], /ENTER,level=1)),bin_factor=bin_factor2[q],res_factor=res_factor2[q]) 
              x2=str2.(xtag_loc)
              y2=str2.(tag_loc)
            endif else if (NOT strmatch(str2_arr[q],'BLANK',/fold_case)) then begin
              str2=(SCOPE_VARFETCH(str2_arr[q], /ENTER,level=1))
              x2=str2.(xtag_loc)
              y2=str2.(tag_loc)
            endif else begin 
              x2=[0,0]
          ...
          if n_elements(str3_arr) eq n_total*m_total then begin
             if n_elements(SCOPE_VARFETCH(str3_arr[q], /ENTER,level=1)) eq 0 then begin
                print,' '
                print,'*** Variable name ',str3_arr[q],' does not exist!!! ***'
                print,' '
                return
             endif
             if (bin_factor3[q] ne 1.) OR (res_factor3[q] ne 1.) then begin
              str3=rebin_reduce_res((SCOPE_VARFETCH(str3_arr[q], /ENTER,level=1)),bin_factor=bin_factor3[q],res_factor=res_factor3[q]) 
              x3=str3.(xtag_loc)
              y3=str3.(tag_loc)
             endif else if (NOT strmatch(str3_arr[q],'BLANK',/fold_case)) then begin
              str3=(SCOPE_VARFETCH(str3_arr[q], /ENTER,level=1))
              x3=str3.(xtag_loc)
              y3=str3.(tag_loc)
            endif else begin 
              x3=[0,0]
          ...
          if n_elements(str4_arr) eq n_total*m_total then begin
             if (bin_factor4[q] ne 1.) OR (res_factor4[q] ne 1.) then begin
              str4=rebin_reduce_res((SCOPE_VARFETCH(str4_arr[q], /ENTER,level=1)),bin_factor=bin_factor4[q],res_factor=res_factor4[q]) 
              x4=str4.(xtag_loc)
              y4=str4.(tag_loc)
             endif else if (NOT strmatch(str3_arr[q],'BLANK',/fold_case)) then begin
              str4=(SCOPE_VARFETCH(str4_arr[q], /ENTER,level=1))
              x4=str4.(xtag_loc)
              y4=str4.(tag_loc)
            endif else begin 
              x4=[0,0]
      ...
      endelse
       ...
       plot,x1,y1/yfactor,psym=10,$
            ytitle=ytitle[n+m],xtitle=xtitle[n+m],$
            charsize=charsize*0.75,thick=thick,$
            ...
            if keyword_set(fill) then begin
              for f=0,n_elements(fill[0,*])-1 do begin
                  if tagfill[f,q] eq f+1 then begin
                      good_fill=where((x1 ge fill[0,f]) AND (x1 le fill[1,f]),num_good)
                      if keyword_set(norm) then bottom=1. else bottom=0.
                      if num_good ne 0 then plotcolorfill,x1[good_fill],y1[good_fill]/yfactor,/noerase,bottom=bottom,color=fsc_color('blk4') 
                             oplot,x1,y1/yfactor,psym=10, color=color[0],thick=thick
                  endif
              endfor
            endif 
       ...
       if keyword_set(x2) then oplot,x2,y2/yfactor,psym=10,color=color[1]
       if keyword_set(x3) then oplot,x3,y3/yfactor,psym=10,color=color[2]
       if keyword_set(x4) then oplot,x4,y4/yfactor,psym=10,color=color[3]
       ...
       if keyword_set(labels) or keyword_set(text) then begin
          pos=[(!p.position[2]-!p.position[0])*6.5/8.0+!p.position[0],$
               (!p.position[3]-!p.position[1])*6.5/8.0+!p.position[1]]   
         ;right
         if (position[q] eq 0) or (position[q] eq 1) then pos[0]=(!p.position[2]-!p.position[0])*6.5/8.0+!p.position[0]
         ;left
         if (position[q] eq 2) or (position[q] eq 3) then pos[0]=(!p.position[2]-!p.position[0])*1.5/8.0+!p.position[0]
         ;top
         if (position[q] eq 0) or (position[q] eq 3) then pos[1]=(!p.position[3]-!p.position[1])*6.5/8.0+!p.position[1] 
         ;bottom
         if (position[q] eq 1) or (position[q] eq 2) then pos[1]=(!p.position[3]-!p.position[1])*1.5/8.0+!p.position[1] 

         if (NOT strmatch(str1_arr[q],'BLANK',/fold_case)) then begin
           if ((SCOPE_VARFETCH(str1_arr[q], /ENTER,level=1)).ion eq 'HI')  then $
              label1=(SCOPE_VARFETCH(str1_arr[q], /ENTER,level=1)).ion else $
              ...
              xyouts,pos[0],pos[1],label1,$
                 align=0.5,/normal,charsize=charsize*0.85,color=color 
 
             if keyword_set(str2) then begin    
               if (SCOPE_VARFETCH(str1_arr[q], /ENTER,level=1)).ion eq 'HI' then $
                 label2=(SCOPE_VARFETCH(str1_arr[q], /ENTER,level=1)).ion else $
                 label2=(SCOPE_VARFETCH(str2_arr[q], /ENTER,level=1)).ion+' '+lambda $
                   +' '+strcompress(string((SCOPE_VARFETCH(str1_arr[q], /ENTER,level=1)).wavc,$
                   format='(I4)'),/re)
   
               if label1 eq label2 then xyouts,pos[0],pos[1],label1,align=0.5,/normal,charsize=charsize*0.85,color=color[1]
             endif
 
              if keyword_set(text) then begin
                 ;top
                 if (position[q] eq 0) or (position[q] eq 3) then pos[1]=(!p.position[3]-!p.position[1])*6.0/8.0+!p.position[1] 
                 ;bottom
                 if (position[q] eq 1) or (position[q] eq 2) then pos[1]=(!p.position[3]-!p.position[1])*2.0/8.0+!p.position[1] 
                 xyouts,pos[0],pos[1],text[q],align=0.5,/normal,color=tcolor,charsize=charsize*0.85
...
if device eq 'X' then ypos = 0.025 else ypos = 0.03-m*0.0001
...
;Add yfactor to the displayed ytitle
   ...
   if (yscalar ne '0') AND (ygood eq 2) AND (NOT keyword_set(norm)) then $
      ...
      else if (yfactor ne '0') AND (ygood eq 1) AND (NOT keyword_set(norm)) $
         then grand_ytitle=temp_ytitle[0]+' x 10!U'+yscalar+'!N' 
endif
if device eq 'X' then xpos = 0.04 else xpos = 0.0275+n*0.01
