...
;----------------------------------------------------------------------------
pro myfit,x,y,xarray,yarray,store,ycon,a,sigma,ycon_sig,ftflag,maxord=maxord
...
;
;	PRINT,'IMYFIT(v6.0)::  Current fit order: ', $
...
IF NOT keyword_set(maxord) THEN BEGIN 
;
;Print heading and instructions.
;
	READ,'IMYFIT(v6.0)::  Minimum order of fit (-1=clear,0=quit): ',minord 
	minord = FIX(minord)  
	IF minord EQ 0 THEN RETURN
	IF minord LT 0 THEN BEGIN
		PLOT,x,y  &  GOTO,LOOP
	ENDIF
	READ,'IMYFIT(v6.0)::  Maximum order of fit: ',maxord  
	maxord = FIX(maxord) > minord
	PRINT,'IMYFIT(v6.0)::  Working...'
...
;
	...
	IF maxord GE N_ELEMENTS(xarray) THEN BEGIN
		if N_ELEMENTS(xarray) ge 3 then begin
			maxord = N_ELEMENTS(xarray)
		endif else GOTO,ESCAPE
	ENDIF 
...
;
	PRINT,'IMYFIT(v6.0)::  Information Dump Follows'
	PRINT,'----------------------------------------'
	...
	PRINT,'----------------------------------------'
	PRINT,'IMYFIT(v6.0)::  End of Information Dump'
        PRINT,'IMYFIT(v6.0)::  Hit any key to continue'
        choice = GET_KBRD(1)
;
