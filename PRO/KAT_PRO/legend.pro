...
;       scale = fractional reduction of line length.
;
...
;       Added scale to modify line length Dec 2012. K Barger
;-
pro legend, items, BOTTOM_LEGEND=bottom, BOX = box, CENTER_LEGEND=center, $
    ...
    OUTLINE_COLOR = outline_color, FONT = font, scale=scale
;
;       =====>> HELP
;
compile_opt idl2
...
;; extra by djseed .. psym=88 means use the already defined usersymbol
 ...
 else  if psym[i] GT 8 then p_sym = symcat(psym[i],thick=thick[i]) else p_sym= psym[i]
...
;    if (num eq 1) and vertical then xp = x + xt/2      ; IF 1, CENTERED.
  ...
  x = x + width*xsign
  if width ne 0 then x = x + 0.5*xspacing*0.5

  if (NOT keyword_set(scale)) then xp1_old=xp
  xyouts,x-(xp1_old-xp[0])*scale,y,items[i],width=width,/norm,color=textcolors[i],size=charsize, $
