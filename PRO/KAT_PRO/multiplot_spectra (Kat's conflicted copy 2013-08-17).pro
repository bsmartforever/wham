pro multiplot_spectra,str1_arr,str2_arr,str3_arr,str4_arr,$
    flux=flux,norm=norm,apparent=apparent,sub=sub,$
    dimensions=dimensions,ygap=ygap,xgap=xgap,$
    device=device,ps=ps,filename=filename,landscape=landscape,portrait=portrait,$
    ...
    labels=labels,left=left,right=right,top=top,bottom=bottom,$
    ...
    xtitle=xtitle,ytitle=ytitle,title=title,wavelength=wavelength,$
    ...
    ymultiple=ymultiple,xsize=xsize,ysize=ysize
...
;
;  str1_arr, str2_arr, str3_arr, str4_arr - String arrays containing the name of the structure containing the spectra. 
...
;  label       - Add ion + wavelength label to all panels. Default position is in the top right corner.
;  right       - Add label to right side. This is default.
;  left        - Add label to left side. 
;  top         - Add label to top of panel. This is default.
;  bottom      - Add label to bottom of panel.
...
; arrow        - Adds an arrow plus number identifier over a spectral feature at a specific xposition and in a specific panel
...
if n_elements(SCOPE_VARFETCH(str1_arr[0], /ENTER,level=1)) eq 0 then begin
   print,' '
...
;Finds the location of the structure tag with name equal to variable. 
if (NOT keyword_set(wavelength)) then $
   xtag_loc=where(strcmp(tag_names((SCOPE_VARFETCH(str1_arr[0], /ENTER,level=1))),'vel',/fold_case) eq 1) $
else xtag_loc=where(strcmp(tag_names((SCOPE_VARFETCH(str1_arr[0], /ENTER,level=1))),'wave',/fold_case) eq 1)
...
if (device eq 'ps') or (device eq 'PS') then begin
   ...
   !x.thick=5
   !y.thick=5
   if (!p.thick le 2) AND (NOT keyword_set(thick)) then begin
      !p.thick=5
      thick=5
   endif else !p.thick=thick
   lambda='!9l!3'
...
if n_elements(ytitle) eq n_elements(str1) then !p.position=[0.1,0.1,0.9,0.95] $
   else !p.position=[0.1,0.1,0.95,0.95]
...
for n=0,n_total-1 do begin 
    for m=0, m_total-1 do begin 
       ...
       !p.position=[ip[2]-(ip[2]-ip[0])/n_total*(n_total-n)+xgap/2.0,$
        ...
        if n_elements(SCOPE_VARFETCH(str1_arr[q], /ENTER,level=1)) eq 0 then begin
           print,' '
       ...
       if (NOT keyword_set(sub)) then begin
          if (bin_factor1[q] ne 1.) OR (res_factor1[q] ne 1.) then $
          ...
          if n_elements(str2_arr) eq n_total*m_total then begin
             ...
             if (bin_factor2[q] ne 1.) OR (res_factor1[q] ne 1.) then $
          ...
          if n_elements(str3_arr) eq n_total*m_total then begin
             ...
             if (bin_factor3[q] ne 1.) OR (res_factor1[q] ne 1.) then $
          ...
          if n_elements(str4_arr) eq n_total*m_total then begin
             if (bin_factor4[q] ne 1.) OR (res_factor4[q] ne 1.) then $
        ...
        axis,xa=1,xminor=xminor,yminor=yminor,$
            ...
            if keyword_set(fill) then begin
              for f=0,n_elements(fill[0,*])-1 do begin
                  if tagfill[f,q] eq 1 then begin
       ...
       if keyword_set(labels) then begin
         ...
         if (position[q] eq 1) or (position[q] eq 2) then pos[1]=(!p.position[3]-!p.position[1])*1.5/8.0+!p.position[1] 
          ...
          if (SCOPE_VARFETCH(str1_arr[q], /ENTER,level=1)).ion eq 'HI' then $
             xyouts,pos[0],pos[1],(SCOPE_VARFETCH(str1_arr[q], /ENTER,level=1)).ion,$
                align=0.5,/normal else $
             xyouts,pos[0],pos[1],(SCOPE_VARFETCH(str1_arr[q], /ENTER,level=1)).ion+' '+lambda $
                +strcompress(string((SCOPE_VARFETCH(str1_arr[q], /ENTER,level=1)).wavc,$
                format='(I4)'),/re),align=0.5,/normal
...
if device eq 'X' then ypos = 0.025 else ypos = 0.03-m*0.01
