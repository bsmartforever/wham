...
;
; Output:
;	Structure containing the galexid, ra (degrees), dec (degrees), glon, glat, vmag (mag), FUV (mag), NUV (mag)
;
; Example:
...
cd, '$HOME/PRO/KAT_PRO/GALEX/',current=proper_directory
...
spawn,'rm -f galexcatalogs_kbarger*.csv'
spawn,'rm -f galexr6plux7_kbarger*.csv'

command=strcompress("java -jar $HOME/PRO/KAT_PRO/GALEX/GALEXCatalogs/casjobs.jar execute -t 'MyDB' -n 'drop query' 'drop table galexcatalogs'")
spawn,command,result
command=strcompress("java -jar $HOME/PRO/KAT_PRO/GALEX/GALEXCatalogs/casjobs.jar execute -t 'MyDB' -n 'drop query' 'drop table galexr6plux7'")
spawn,command,result
;End of house cleaning

command= $
	strcompress("java -jar $HOME/PRO/KAT_PRO/GALEX/GALEXCatalogs/casjobs.jar run -t 'GALEXCatalogs' -n 'test query' " + $ 
    "'select p.objid, nb.distance as dstArcMin, p.ra, p.dec, p.glon, p.glat, p.fuv_flux as fuv, p.nuv_flux as nuv into mydb.galexcatalogs from bcscat_ais as p,"+ $
...
if rows ne 0 then begin
	command=strcompress("java -jar $HOME/PRO/KAT_PRO/GALEX/GALEXCatalogs/casjobs.jar extract -b galexcatalogs -F -d -r")
	spawn,command,result
	...
	tmp=read_csv('galexcatalogs_kbarger*.csv')
...
;		struct_galex[i].id=fix(tmp[0])
;		struct_galex[i].dist=double(tmp[1])
...
command= $
	strcompress("java -jar $HOME/PRO/KAT_PRO/GALEX/GALEXCatalogs/casjobs.jar run -t 'GALEXGR6Plus7' -n 'test query' " + $ 
	" 'select nb.objid,nb.distance as dstArcMin,p.ra,p.dec,p.glon,p.glat,p.nuv_flux,p.fuv_flux into mydb.galexr6plux7 "+$
...
if rows ne 0 then begin
	command=strcompress("java -jar $HOME/PRO/KAT_PRO/GALEX/GALEXCatalogs/casjobs.jar extract -b galexr6plux7 -F -d -r")
	spawn,command,result
	check=''
	while check eq ''  do spawn,'find . -type f -size +1c |grep -i csv',check,error
	tmp=read_csv('galexr6plux7_kbarger*.csv')
	;For some crazy reason, the code makes it to here even though the csv file contains no data...
	;The nrows check should account for this, but it's not for some reason...
	if size(tmp,/type) eq 8 then begin
...
;	
;	tmp=strsplit(result[3],',',/extract)
;	struct_gr7=replicate({id:0,dist:0.0D,ra:0.0D,dec:0.0D,glon:0.0D,glat:0.0D,fuv:0.0,nuv:0.0},n_elements(result))
;	struct_gr7=struct_conv(struct_gr7)
;	
;	for i=0,n_elements(result)-1 do begin
;		tmp=strsplit(result[i],',',/extract)
;		struct_gr7[i].id=fix(tmp[0])
;		struct_gr7[i].dist=double(tmp[1])
;		struct_gr7[i].ra=double(tmp[2])
;		struct_gr7[i].dec=double(tmp[3])
;		struct_gr7[i].glon=double(tmp[4])
;		struct_gr7[i].glat=double(tmp[5])
;		struct_gr7[i].fuv=float(tmp[6])
;		struct_gr7[i].nuv=float(tmp[7])
;	endfor
