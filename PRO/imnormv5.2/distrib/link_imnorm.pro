...
RESET:
;
;This section declares any parameters that need to be defined before
;beginning main program execution.  The variables are grouped
...
;execution.  This section plots the current plot, displays a command
;prompt, and waits for a command character to be entered.
; 
...
;------------------------------------------------------------------------------
LOOP1:
;This section deals with the selection of a continuum for later
;polynomial fitting and normalization.  The user defines regions of
;the spectrum to be used as continuum.  The regions are stored in the
;array store.  IMCONT is called to do the continuum definition.
;
;Make sure that the continuum definition is done in velocity space
;rather than in wavelength space.  This keeps the matrix resulting
;from the polynomial fit from becoming singular.
;
...
;------------------------------------------------------------------------------
LOOP2:
;This section is an extension of the previous section.  It fits a
;polynomial to the region defined in LOOP1.  IMYFIT is called to do
;the fit.
;
;Make sure that the continuum fitting is done in velocity space rather
;than in wavelength space.  This keeps the matrix resulting from the
;polynomial fit from becoming singular.
;
	IF axflag NE -1 THEN GOTO,CONTROL1
...
;Added by jch to correct for lingering plot titles/ranges.
        !x.range=[0,0] & !y.range=[0,0] 
	!x.title=' ' & !y.title=' ' & !p.title= ' ' 
