...
;       8/29/03. Added REVERSE keyword to reverse the byte-scale image
;                colors (GJM)
;       5/22/13: Modified colorbar colors to table indices 0-254. This is so 255 can equal white
;                 and 254 to equal the variable color to match the the colors used by whammap.pro. 
;                 The default is for color=black. -KAT
;-
;
;###########################################################################
;
...
PRO COLORBAR, BOTTOM=bottom, CHARSIZE=charsize, COLOR=color, DIVISIONS=divisions, $
   ...
   RANGE=range, FONT=font, TICKLEN=ticklen, REVERSE = reverse, _EXTRA=extra
...
;Setting this to format = '(I5)' is pretty stupid. 
;This messes up the display all decimal tick values
;IF N_ELEMENTS(format) EQ 0 THEN format = '(I5)'
;if keyword_set(ylog) then format='exponent'
...
IF N_ELEMENTS(range) NE 0 THEN BEGIN
   ...
   maxrange = range[1]
ENDIF
;Forcing divisions = 6 is stupid. IDL automatically
;selects tick divisions between 3 and 6 if the number 
;of ticks is not set. These IDL selected values usually 
;are divided much prettier than a forced 6 value.
;A forced 6 value makes it hard to have clead looking
;automated plots.
...
ENDELSE
 ...
 bar = BYTSCL(bar, TOP=(ncolors-2 < (255-bottom))) + bottom

 IF keyword_set(reverse) THEN bar =  255 - bar
...
IF KEYWORD_SET(vertical) THEN BEGIN
   ...
   IF KEYWORD_SET(right) THEN BEGIN
      ...
      PLOT, [minrange,maxrange], [minrange,maxrange], /NODATA, XTICKS=1, $
         ...
         POSITION=position, COLOR=!p.color, CHARSIZE=charsize, /NOERASE, $
      ...
      AXIS, YAXIS=1, YRANGE=[minrange, maxrange], YTICKFORMAT=format, YTICKS=divisions, $
         YTICKLEN=ticklen, YSTYLE=1, COLOR=!p.color, CHARSIZE=charsize, $
   ...
   ENDIF ELSE BEGIN
      ...
      PLOT, [minrange,maxrange], [minrange,maxrange], /NODATA, XTICKS=1, $
         ...
         POSITION=position, CHARSIZE=charsize, /NOERASE, $
         YTICKFORMAT=format, XTICKFORMAT='(A1)', YTICKLEN=ticklen , $
         YRANGE=[minrange, maxrange], FONT=font, YTITLE=title, _EXTRA=extra,color=!p.color
...
ENDIF ELSE BEGIN
   ...
   IF KEYWORD_SET(top) THEN BEGIN
      ...
      AXIS, XTICKS=divisions, XSTYLE=1, COLOR=!p.color, CHARSIZE=charsize, $
