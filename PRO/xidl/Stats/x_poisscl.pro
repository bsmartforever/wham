...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
function x_poisscl, x, cl, SIGMA=sigma, NSPL=nspl

  ; 
  if  N_params() LT 1  then begin 
      print,'Syntax - ' + $
        'prob = x_poisscl( x, [cl], SIGMA= ) [v1.1]'
      return, 0.
  endif 
  if not keyword_set( NSPL ) then nspl = 1000L
  if keyword_set( SIGMA ) then cl = gauss_pdf(sigma)

  if not keyword_set(CL) then stop
