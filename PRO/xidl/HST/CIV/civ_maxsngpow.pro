...
function civ_maxsngpow_int_freqdistr, var
  ...
  common civ_maxsngpow_cmmn, sngpow_intlim, sngpow_data, sngpow_sigdata, $
     ...
     sngpow_datatype, sngpow_flg, sngpow_datanorm, qromb_par

  if sngpow_datatype eq 'NCOLM' then $
     x_var = civ_sensitivity_x(sngpow_cumx,ncolm=alog10(var)) $
  else x_var = civ_sensitivity_x(sngpow_cumx,ew=var) 
...
pro civ_maxsngpow, strct_fil, sens_fil, guess, siiv=siiv, $
  ...
  ;;  sngpow_flg, sngpow_datanorm, qromb_par)
  common civ_maxsngpow_cmmn 
  ...
  if keyword_set(erronly) then begin
     ...
     sngpow_coeff = alog(fitstrct.coeff*sngpow_datanorm^fitstrct.alpha)
     ...
     ;; Sort for kstest
     ion_indx = where(strtrim(civcand.ion,2) ne '',dblt_flag)
     ion_indx = ion_indx[0]
     if keyword_set(ew) then begin
        sngpow_datatype = 'EW'
        sngpow_datanorm = 400.d  ;130.d
        ...
        sngpow_xdata = civ_sensitivity_x(sngpow_cumx,ew=sngpow_data) 
     ...
     if keyword_set(ncolm) then begin
        ...
        sngpow_xdata = civ_sensitivity_x(sngpow_cumx,ncolm=alog10(sngpow_data))
        if not keyword_set(ostrct_fil) then $
  ...
  errsurf = where(exp(logL_grid-logL_mx) ge exp(-0.5),nerrsurf)
  indx = lonarr(nerrsurf,2)
  for ii=0,nerrsurf-1 do indx[ii,*] = array_indices(logL_grid,errsurf[ii])
  ...
  if keyword_set(plot) then begin
     ...
     contour,exp(logL_grid-logL_mx),coeff_grid,alpha_grid,levels=[exp(-0.5)]
