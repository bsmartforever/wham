...
pro civ_velplt, strct_fil, instr_list, NTOT=NTOT, CSIZE=csize, $
                ...
                PSROOT=psroot, XTINT=xtint,LAPTOP=laptop, $
                OUTLINE=OUTLINE, LABEL=LABEL, hdf5_fil=hdf5_fil, $
                BIN=BIN,YMNX=YMNX, vmnx=vmnx, CONCAT=CONCAT, $
                PLTTITLE=plttitle, FIXVMNX=fixvmnx, $
                PLTERR=plterr, PLT_AOD=plt_aod,_extra=extra
  ...
  if not keyword_set(plt_aod) then plt_aod = 1 ; yes
...
;        ytint = round(10*ytint)/10.
        ...
        if keyword_set(label) then begin
           ...
           wobs = strct[ss].wrest[lin[ii]]*(1.+strct[ss].zabs[lin[ii]])
     ...
     if plt_aod eq 1 then $
        xyouts, 0.05, 0.6, 'Normalized Flux', $
                alignment=0.5, ORIENTATION=90., /normal, charsize=sv_csize $
     else $
        xyouts, 0.05, 0.5, 'Normalized Flux', $
                alignment=0.5, ORIENTATION=90., /normal, charsize=sv_csize

     if keyword_set(plttitle) then begin
        xyouts,0.56,0.97,plttitle[ss],charsize=LSIZE,alignment=0.5,/normal
     endif else begin
        ;; Lya Forest tag
        if (strct[ss].flg_sys[0] and 8) eq 0 then $
           xyouts,0.5*(0.88+0.25),0.97,'Ly!9a!X Forest',$
                  color=clr.red,charsize=0.7*LSIZE, alignment=0,/normal
        ...
        xyouts,0.12,0.97,dum,charsize=0.7*LSIZE,alignment=0,/normal
        
        ;; Reshift label upper right corner
        xyouts,0.98,0.97,'!8z!X!D1548!N = '+$
               strtrim(string(strct[ss].zabs[lin[0]],format='(f8.5)'),2),$
               charsize=0.7*LSIZE,alignment=1.0,/normal
        ;; Print input file name in upper left corner
        ;; Also print flg_sys
        sysflg = ' ('+strtrim(strct[ss].flg_sys[0],2)+$
                 ', '+strtrim(strct[ss].flg_sys[1],2)+')'
        if keyword_set(psroot) and not keyword_set(concat) then begin
           if size(psroot,/type) eq 7 then begin
              dum = [strpos(psfile,'/',/reverse_search)+1,$
                     strpos(psfile,'.',/reverse_search)]
              xyouts,0.12,0.97,strmid(psfile,dum[0],dum[1]-dum[0])+sysflg,$
                     charsize=0.7*LSIZE,alignment=0,/normal
           endif else begin
              dum = strtrim(strct[ss].qso,2)
              xyouts,0.12,0.97,dum+sysflg,$
                     charsize=0.7*LSIZE,alignment=0,/normal
           endelse          
        endif else begin
           dum = strtrim(strct[ss].qso,2)
           xyouts,0.12,0.97,dum+sysflg,charsize=0.7*LSIZE,alignment=0,/normal
        endelse 
     endelse                     ; /label
     ...
     if (strct[ss].flg_sys[0] and 8) eq 0 then $
        xyouts,0.5*(0.88+0.25),0.97,'Ly!9a!X Forest',$
               color=clr.red,charsize=0.7*LSIZE, alignment=0.5,/normal
     ...
     if plt_aod eq 1 then begin
        ...
        ;; Tick interval
        ytint = (aodcolmnx[1]-aodcolmnx[0])/3
        ytint = round(10*ytint)/10.
        ...
        dum = 'Log N('+elm+'!E+'+strtrim(ion-1,2)+'!N)'
        plot, vmnx, aodcolmnx, xrange=vmnx, $
              yrange=aodcolmnx, xmargin=xmrg, ymargin=[0,0], /NODATA, $
              charsize=csize, psym=10, background=clr.white, $
              color=clr.black, ytickinterval=ytint, $
              xstyle=1, ystyle=ysty, thick=lthick, xtickinterval=xtint, $
        ...
        oplot,strct[ss].aodm_vel[*,1]+dv,strct[ss].aodm_civ[*,1],$
              psym=10,color=clr.red,thick=lthick
