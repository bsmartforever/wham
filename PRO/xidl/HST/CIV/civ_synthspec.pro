...
for ispec=0,nlist-1 do begin
    ...
    civ_find,auto_fil,siiv=siiv,root=civrec_root,zlim=dzspec,instrfil=synthinstr_fil
...
pro civ_synthspec,qso,zqso,siiv=siiv,outdir=outdir,nabs=nabs,$
                  ...
                  config_fil=config_fil,savfits=savfits,debug=debug,$
  ...
  for ispec=0,nlist-1 do begin
     ...
     if (nbinz-1)*config_strct.binz+dzspec[0] ge dzspec[1] then $
        nbinz = nbinz - 1
     mxsep = round(config_strct.dvabs[1]-config_strct.dvabs[1] + $
        ...
        for iabs=0,nabs-1 do begin
           ...
           for iz=0,nzsep-1 do begin
              ...
              if keyword_set(resume) and ntest ne 0 then $
                 ...
                 searchspec,synthspec.wave,synthspec.flux,synthspec.error,3.,dopb,$
                 ...
                 if size(auto,/type) eq 8 then begin
                    civ_find,auto,ostrct=civstrct,zlim=dzspec,siiv=siiv,$
                             inspec=synthspec,instr_flg=2^ispec,_extra=extra ;/silent
                    if size(civstrct,/type) eq 8 then begin
                       ;; Other post-processing
                       civstrct.qso = qso
                       civstrct.zqso = zqso
     ...
     civ_complete,trufits[0:nfits-1],recfits[0:nfits-1],ostrct_fil=ostrct_root+'.fits',$
                  binncolm=config_strct.binncolm, binz=config_strct.binz, siiv=siiv,$
     ...
     civ_complete_plot,ostrct_root+'.fits','cmpltn' + prs[nprs-1],0,yrng=[12.6,14.4],$
                       /pltnhist,siiv=siiv 
     civ_complete_plot,ostrct_root+'.fits','cmpltew' + prs[nprs-1],1,yrng=[30.,200.],$
                       /pltnhist,siiv=siiv 
