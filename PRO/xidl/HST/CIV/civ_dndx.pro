...
pro civ_dndx_plot,strct_fil, sens_fil, psfil, siiv=siiv, ncolm=ncolm, $
  ...
  if keyword_set(siiv) then begin
     if keyword_set(ncolm) then begin
        ytitle = 'd!8N!X/d!8X!X (N(Si!E+3!N) '+ge_sign+' N!Dlim!N)'
        xtitle = 'log N!Dlim!N'
     endif else begin
        ytitle = 'd!8N!X/d!8X!X (W!Dr,1393!N '+ge_sign+' W!Dlim!N)'
        xtitle = 'W!Dlim!N (m'+STRING("305B) +')' 
     endelse 
  endif else begin
     if keyword_set(ncolm) then begin
        ytitle = 'd!8N!X/d!8X!X (N(C!E+3!N) '+ge_sign+' N!Dlim!N)'
        xtitle = 'log N!Dlim!N'
     endif else begin
        ytitle = 'd!8N!X/d!8X!X (W!Dr,1548!N '+ge_sign+' W!Dlim!N)'
        xtitle = 'W!Dlim!N (m'+STRING("305B) +')' 
     endelse 
  endelse 
  ...
  if keyword_set(fit_fil) then begin
     ...
     if keyword_set(label) then begin
        ...
        xyouts, xloc*dx + xrng[0], 0.88*dy + yrng[0], $
                'k = '+strtrim(string(fitstrct.coeff,format='(e9.2)'),2),$
                charsize=csize, color=clr.black 
...
pro civ_dndx, rslt, sens_fil, strct_fil, siiv=siiv, data=data, xdata=xdata,$
              ...
              sigcoeff_dndx=sigcoeff_dndx, sigalpha_dndx=sigalpha_dndx,_extra=extra
...
;        stop,'civ_dndx: if want fit dN/dX errors, input file' 
     ...
     for ii = 0,nbin-1 do begin
        ...
        rslt[ii,0] =  -fitstrct.coeff/alpha1 * $
        ...
        if keyword_set(errstrct) then begin
           ...
           for nn=0L,n_elements(errstrct.surf)-1 do begin 
              ...
              tmp.coeff = errstrct.coeff_grid[mm[0]] 
              tmp.alpha = errstrct.alpha_grid[mm[1]] 
              if keyword_set(ncolm) then $
                 civ_dndx, tmprslt, siiv=siiv, ncolm=data[ii], fit_fil=tmp $ ; won't call this section
              else civ_dndx, tmprslt, siiv=siiv, ew=data[ii], fit_fil=tmp 
              dndx[nn] = tmprslt[0] 
           endfor
           dndx_mn = min(dndx,imn,max=dndx_mx,subscript_max=imx) ; find extrema
