...
pro civ_group,subciv_fil, civstrct_fil, rating=rating,flg_sys=flg_sys,$
  ...
  if not keyword_set(civstrct_fil) then $
  ...
  if keyword_set(rating) and nciv gt 0 then begin
     ...
     for ii=0,nrtg-1 do begin
        gd = where(civstrct.rating_eye eq rating[ii])
  ...
  if keyword_set(zlim) and nciv gt 0 then begin
     gd = where(civstrct.zabs[indx] ge zlim[0] and civstrct.zabs[indx] lt zlim[1],nciv)
     if nciv eq 0 then begin
  ...
  if keyword_set(ewlim) and nciv gt 0 then begin
     gd = where(civstrct.ew[indx] ge ewlim[0] and civstrct.ew[indx] lt ewlim[1],nciv)
     if nciv eq 0 then begin
  ...
  if keyword_set(nlim) and nciv gt 0 then begin
     ncolm = civ_calcewn_ndblt(civstrct,dblt_name,/log,flg_colm=flg_colm,/silent)
