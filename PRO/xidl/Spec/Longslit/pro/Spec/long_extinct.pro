...
ENDIF ELSE IF strmatch(sxpar(scihdr, 'DETECTOR'), 'mars*') OR $
   ...
   strmatch(sxpar(scihdr, 'TELESCOP'), '*CA-3.5*') $
THEN BEGIN
...
IF KEYWORD_SET(extinctfile) THEN $
  readcol, extinctfile, wave_ext, mag_ext, format = 'F,F' 
...
IF KEYWORD_SET(MAG_EXT) and not keyword_set(NOEXTINCT) THEN BEGIN
    ...
    ext = 10.0D^(0.4D*mag_ext1*airmass)/exptime
ENDIF 
