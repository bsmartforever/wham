...
; Plot Data
  ...
  if state.flg_ytwo EQ 1 AND state.flg_smooth2 EQ 0 then $
    oplot, state.wave_two, state.ytwo, psym = state.psym, color = clr.purple $
  ELSE if state.flg_ytwo EQ 1 AND state.flg_smooth2 GT 0 THEN $
    oplot, state.wave_two, state.smooth2, psym = state.psym, color = clr.purple 

  ;; YTHREE
  if state.flg_ythree EQ 1 THEN $
     oplot, state.wave_three, state.ythree, psym = state.psym, color = clr.blue
...
common x_specplot_lines
  ...
  clr = getcolor(/load)
  oplot, wav*(1+zabs), fx*nrm, color=clr.blue, psym = 10
...
pro x_specplot_pltgalt, state
  ...
  oplot, state.wave, fx_rebin*nrm, color = clr.green, psym = 10
...
;                                   /double)/(1.+zabs)
          ...
          dwv = abs(dwv)
          state.EW = total(1.-(state.fx[sumpix]>0.)/cntm)*dwv / $
            (1.+zabs)
          ;; ERROR
...
pro x_specplot, flux, ysin, XSIZE=xsize, YSIZE=ysize, TITLE=title $
                ...
                , YTWO = ytwo, TWO_WAVE = WAVE_TWO , ZOUT=zout $
                , YTHREE = YTHREE, THREE_WAVE = WAVE_THREE $
                , TWO_IFLG=two_iflg, GALTEMPL_FIL=galtempl_fil $
...
;
  if  N_params() LT 1 and not keyword_set(GUI) then begin 
    ...
    print, '            /QAL, /GAL, /QSO, ZIN=, /BLOCK, /NRM, /LLS, /QSO, /AIR, /AUTO, YTWO= ) [v1.1]'
    return
  endif 
...
;      Lines
  state.lines_id = WIDGET_LIST(toolbar, $
                             VALUE=['DLA','GAL','QSO','LLS','H2','GRB','LBG'], $
