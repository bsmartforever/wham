...
;
; <p>Only spectral line data can currently be fetched using an nsave number.
;
; <p>nget fetchs data from the output file (keep file) unless infile is set.
; <p>
...
;
; @param nsave {in}{required}{type=long} nsave number to be retrieved
; @keyword buffer {in}{optional}{type=long} global buffer number where the retrieved spectrum is stored (defaults to 0).
; @keyword infile {in}{optional}{type=boolean} if set, use line input file instead of the output file.
...
; @keyword ok {out}{optional}{type=boolean} status output
;
...
; </pre>
;
; @version $Id: nget.pro,v 1.9 2006/05/17 07:11:14 bgarwood Exp $
;-
