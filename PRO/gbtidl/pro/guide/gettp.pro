...
;
; @version $Id: gettp.pro,v 1.40 2011/05/12 15:50:20 bgarwood Exp $
;-

pro gettp,scan,ifnum=ifnum,intnum=intnum,plnum=plnum,fdnum=fdnum,sampler=sampler,eqweight=eqweight, $
          tcal=tcal,sig_state=sig_state,cal_state=cal_state,quiet=quiet,keepints=keepints,useflag=useflag, $
   ...
   ; Get the scan_info for the scan
    ...
    for i = 0,(nIntegrations-1) do begin
        if doTPcal then begin
            dototalpower,result,data[sigoff[i]],data[sigon[i]],tcal=tcal
        endif else begin
            data_copy,data[i],result
        endelse
        if doSingleCal then begin
            if cal_state eq 0 then begin
                data[sigoff[i]].tsys = result.tsys
                dcaccum,thisaccum,data[sigoff[i]],weight=weight
            endif else begin
                data[sigon[i]].tsys = result.tsys
                dcaccum,thisaccum,data[sigon[i]],weight=weight
            endelse
        endif else begin
        ...
        if doTPcal and keyword_set(keepints) then begin
            ...
            if doSingleCal then begin
                if cal_state eq 1 then begin
                    tmp = data[sigoff[i]]
                    data_copy, data[sigon[i]], tmp
                    data[sigoff[i]] = tmp
                endif
                    ; cal_state = 1 is sigoff, which is all 
