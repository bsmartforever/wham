...
;+
; Method takes in two arrays and ANDS them so that only the shared
; indicies remain.  This is used to AND different searches in the 
; index file
; @param values_result {in}{required}{type=array} array of integers representing the results of a search
; @param and_result {out}{required}{type=array} array of integers representing the AND of values_result and itself.
; @private
;-
PRO INDEX_FILE::and_results, values_result, and_result
    compile_opt idl2

    ; if either input array is -1, then this is our result
    if (n_elements(values_result) eq 1) then begin 
        if (values_result eq -1) then begin 
            and_result = -1
            return
    ...
    ; find the intersection of the two arrays (this is the AND)
    intersection = [-1]
    for i = 0, (n_elements(values_result)-1) do begin
        result = values_result[i]
        count = 0
        matched_index = where(and_result eq result, count)
        if (count ne 0) then intersection = [intersection,and_result[matched_index]]
    endfor
    ; if there was a result from the AND, take off the init value
    if (size(intersection,/dimensions) gt 1) then $ 
        intersection = intersection[1:(n_elements(intersection)-1)]
    
    ; pass back the AND result
    and_result = intersection
...
FUNCTION INDEX_FILE::search_range, start, finish, search_result
    ...
    if (n_elements(start) ne 0) and (n_elements(finish) ne 0) then begin
        ...
        range_index = lindgen(finish-start+1)
        range_index = range_index + start
        self->find_values_plus_and,(*self.row_lines).index,range_index,search_result
    endif
...
FUNCTION INDEX_FILE::validate_search_keywords, keyword_struct
    ...
    ; what are the keywords allowed? + UNIQUE
    searchkeys = [self->get_available_columns(),"UNIQUE"]
