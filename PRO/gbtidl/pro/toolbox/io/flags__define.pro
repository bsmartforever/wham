...
;-
PRO FLAGS::unflag, id, status, all=all
    compile_opt idl2, hidden
    ...
    if keyword_set(all) then begin
        ...
        if idtype eq 7 then begin
            self->unflag_idstring, id, status
        endif else begin
            self->unflag_id, id, status 
        endelse
    endelse
...
; @param status {out}{optional}{type=long} 0 - failure, 1 - success
;-
PRO FLAGS::unflag_idstring, idstring, status
    compile_opt idl2, hidden
    ...
    if not id_found then message, "IDSTRING could not be found to unflag: "+idstring, /info
...
; them to their new values relative to the current index (remember that this is ; a no-op if there is only one sdfits file).  These values are stored in each
; respective flag file object.
...
FUNCTION FLAGS::get_index_value_recnums, status
    ...
    if n_elements(*self.flag_files) eq 1 then begin
        ...
        ; for multiple files, if one returns an OK status it's all OK
        allStatus = 0
        r = -1
        for i=0,n_elements(*self.flag_files)-1 do begin
