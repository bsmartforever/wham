...
;
; @version $Id: online_status_file__define.pro,v 1.1 2011/04/14 16:41:04 bgarwood Exp $
;
; @private_file
;-
...
; @returns the status of the most recent spectral line file (acs or sp)
; @private
;-
FUNCTION ONLINE_STATUS_FILE::get_all_infos, acs, dcr, sp, zpec, status
    compile_opt idl2, hidden
    ...
    if (status eq 1) then begin
        ...
        ; find most recent of sp, acs
        if strlen(sp.file) eq 0 then begin
            ; assume ACS - may also be empty
            mostRecent = acs
        endif else begin
            if strlen(acs.file) eq 0 then begin
                ; assume SP
                mostRecent = sp
            endif else begin
                if acs.age le sp.age then begin
                    mostRecent = acs
                endif else begin
                    mostRecent = sp
                endelse
...
; @private
;-
FUNCTION ONLINE_STATUS_FILE::get_filenames, acsf, dcrf, spf, zpecf, status
    compile_opt idl2, hidden

    mostRecent = self->get_all_infos(acs,dcr,sp,zpec,status)
    mostRecentFile = ''
