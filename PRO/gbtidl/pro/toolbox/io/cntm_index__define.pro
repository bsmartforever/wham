...
;
;-
FUNCTION CNTM_INDEX::search_index, start, finish, SEARCH=search, INDEX=index, PROJECT=project, FILE=file, EXTENSION=extension, FIRSTROW=firstrow, NUMROWS=numrows, STRIDE=stride, SOURCE=source, PROCEDURE=procedure, OBSID=obsid, SCAN=scan, PROCSEQN=procseqn, E2ESCAN=e2escan, POLARIZATION=polarization, IFNUM=ifnum, TRGTLONG=trgtlong, TRGTLAT=trgtlat, SIG=sig, CAL=cal, NSAVE=nsave
...
FUNCTION CNTM_INDEX::get_scan_info, scan_number, file, count=count, quiet=quiet
    ...
    for j=0,(n_elements(uniqueTimes)-1) do begin
        ...
        for k=0,(n_elements(uniqueFiles)-1) do begin
            ...
            scan_info[count].file = theseFileRows[0].file
