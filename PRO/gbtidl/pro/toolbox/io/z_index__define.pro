...
; @keyword procseqn {in}{optional}{type=long} Procesure sequence
; number
...
FUNCTION Z_INDEX::parse_extension_rows, rows, proj, file_name, ext, missing, virtuals, start
    ...
    while (startRow lt nrows) do begin
        ...
        for j=0,number_of_rows-1 do begin
            ...
            row_info.elevation = self->get_row_value(row,'ELEVATION',virtuals,names,dd )
            row_info.longitude = self->get_row_value(row,'LONGITUDE',virtuals,names,dd )
            row_info.latitude = self->get_row_value(row,'LATITUDE',virtuals,names,dd )
            ...
            row_info.trckbeam = self->get_row_value(row,'TRCKBEAM',virtuals,names,ds )
...
FUNCTION Z_INDEX::get_scan_info, scan_number, file, count=count, quiet=quiet
    ...
    scan_info_struct = {mc_scan:0L,procseqn:0L,timestamp:'',file:'', procedure:'',$
                        n_integrations:0L, $
                        n_cal_states:0L, $
    ...
    for j=0,(count-1) do begin
        ...
        scan_info[j].procseqn = theseFileRows[0].procseqn
...
FUNCTION Z_INDEX::spectrum_to_info, zdc, index, file_name, extension, row_num
    ...
    row_info.procseqn = -1 ;spectrum.procseqn
