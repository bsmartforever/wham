...
;
; @version $Id: scalevals.pro,v 1.4 2005/06/21 17:25:50 bgarwood Exp $
;-
pro scalevals, thevalues, scaledvalues, prefix
    ...
    if (thevalues[0] gt 1e9) then begin
        scaledvalues = thevalues / 1e9
        prefix = 'G'
    endif else begin
        if (thevalues[0] gt 1e6) then begin
            scaledvalues = thevalues / 1e6
            prefix = 'M'
        endif else begin
            if (thevalues[0] gt 1e3) then begin
