...
pro gapnan_i, ii, yy
  ...
  y1 = dblarr(n_elements(yy) + ngti - 1)
  y1(*) = !values.d_nan
  for i = 0, ngti-1 do begin
      i0 = ii(0,i) & i1 = ii(1,i)
      y1(i0+i:i1+i) = yy(i0:i1)
  endfor
  yy = y1
end
...
pro gapnan, tt, y1, y2, y3, y4, y5, maxgap=maxgap, gti=gti, include=include
  ...
  if n_elements(maxgap) GT 0 then begin
      ...
      wh1 = gtiwhere(tt, maxgap, include=include, intervals=intv1, count=ct1)
      intv = lonarr(nt) - 1
  ...
  gapnan_i, ii, tt
  if n_params() GT 2 then gapnan_i, ii, y1
  if n_params() GT 3 then gapnan_i, ii, y2
  if n_params() GT 4 then gapnan_i, ii, y3
  if n_params() GT 5 then gapnan_i, ii, y4
  if n_params() GT 6 then gapnan_i, ii, y5
