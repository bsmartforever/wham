; $Id: cw_animate.pro,v 1.34 2000/09/19 23:56:09 chris Exp $
  ...
  compile_opt hidden
   ...
   CASE event.id OF
      ...
      state.wCyclePlayButton : BEGIN
       ...
       state.wRetreatOneButton: BEGIN
         WIDGET_CONTROL, state.wFramesIndicatorSlider, GET_VALUE = temp
         IF temp GT 0 THEN BEGIN
           temp = temp - 1
           WIDGET_CONTROL, state.wFramesIndicatorSlider, SET_VALUE = temp
           IF (temp NE state.curframe) THEN BEGIN
             SWIN = !d.window
             WSET, state.draw_win
             state.curframe = temp
       ...
       state.wAdvanceOneButton: BEGIN
         WIDGET_CONTROL, state.wFramesIndicatorSlider, GET_VALUE = temp
         IF temp LT (state.nframes-1) THEN BEGIN
           temp = temp + 1
           WIDGET_CONTROL, state.wFramesIndicatorSlider, SET_VALUE = temp
           IF (temp NE state.curframe) THEN BEGIN
             SWIN = !d.window
      ...
      state.wMPEGButton : BEGIN
         mpegFilebase=DIALOG_PICKFILE(/WRITE)
         IF STRLEN(mpegFilebase) GT 0 THEN BEGIN
            tmpBase=WIDGET_BASE(TITLE='Animation: Creating .FLI file', $
                       /FLOATING, MODAL=WIDGET_INFO(event.top, /MODAL), $
                       GROUP=event.top, TLB_FRAME_ATTR=27, /FRAME)
            tmpMsg = 'Creating Frames for FLI movie file:'
            tmpText=WIDGET_TEXT(tmpBase, $
               XSIZE= ( STRLEN(tmpMsg) > STRLEN(mpegFilebase) )+5, $
               YSIZE=4, $
               /ALIGN_CENTER, $
               VALUE=['', tmpMsg, mpegFilebase+'.fli', '0% Done'])
            WIDGET_CONTROL, tmpBase, /REALIZE
            WIDGET_CONTROL, /HOURGLASS
            ;;mpegID = OBJ_NEW('IDLgrMPEG', FILENAME = mpegFilename)
            saveWindow = !D.WINDOW
            DEVICE, GET_VISUAL_NAME=visualName
            mpegFilename = STRARR(N_ELEMENTS((*state.pwinHdl)))
            FOR i = 0, N_ELEMENTS(mpegFilename)-1 DO BEGIN
              mpegFilename[i] = mpegFilebase + '_'+STRTRIM(i, 2)+'.ppm'
            ENDFOR
            CASE visualName OF
                'TrueColor': BEGIN
                    FOR i=0, N_ELEMENTS((*state.pwinHdl))-1 DO BEGIN
                        IF (*state.pwinHdl)[i] GT -1 THEN BEGIN
                            ...
                            FOR icolor = 0, 2 DO BEGIN
                              FOR x = 0, N_ELEMENTS(image[icolor, *, 0])-1 DO BEGIN
                                image2[icolor, 639-x, *] = image[icolor, x, *]
                              ENDFOR
                              FOR y = 0, N_ELEMENTS(image[icolor, 0, *])-1 DO BEGIN
                                image2[icolor, *, 479-y] = image[icolor, *, y]
                              ENDFOR
                            ENDFOR
                            image = image2
                            WIDGET_CONTROL, tmpText, $
                              SET_VALUE = ['', tmpMsg, mpegFilebase+'.fli', $
                                           STRTRIM(ROUND( (100*(i+1)) / N_ELEMENTS((*state.pwinHdl))), 2) + $
                                           '%  Done']
                            WRITE_PPM, mpegFilename[i], Image
            ...
            ;;MPEG_SAVE, mpegID
            ;;MPEG_CLOSE, mpegID
            OPENW, unit, '/tmp/list.txt', /GET_LUN
            FOR i = 0, N_ELEMENTS((*state.pwinHdl))-1 DO PRINTF, unit, mpegFilename[i]
            CLOSE, unit &  FREE_LUN, unit
            Temp = FINDFILE(mpegFilebase + '.fli', Count = FLI_File_Exists)
            IF FLI_File_Exists THEN SPAWN, '/bin/rm ' + mpegFilebase + '.fli'
            SPAWN, 'ppm2fli -g640x480 -s 500 /tmp/list.txt ' + mpegFilebase + '.fli > /dev/null'
            FOR i = 0, N_ELEMENTS((*state.pwinHdl))-1 DO SPAWN, '/bin/rm '+ mpegFilename[i]
...
function CW_ANIMATE, parent, sizex, sizey, nframes, $
  ...
  COMMON BitmapButtons
   ...
   wVCRButtonBase_2 = WIDGET_BASE(wControlBase, /ROW)
   wRetreatOneButton = WIDGET_BUTTON(wVCRButtonBase_2, VALUE = '/usr/users/madsen/wham/pro/left.bmp', $
       UNAME=uname+'_RETREAT', /BITMAP)
   wAdvanceOneButton = WIDGET_BUTTON(wVCRButtonBase_2, VALUE = '/usr/users/madsen/wham/pro/right.bmp', $
       UNAME=uname+'_ADVANCE', /BITMAP)
   ...
   PRINT, cmapApplies,  redrawRequired
   ...
   ELSE $
      wOpenButton = 0
   wMPEGButton = WIDGET_BUTTON(wButtonBase, VALUE='Write FLI', $
       UNAME=uname+'_MPEG')
   if (LMGR(/DEMO) or (NOT LMGR('idl_mpeg',VERSION='1.0'))) then $
       WIDGET_CONTROL, wMPEGButton, SENS=1
   ...
   WIDGET_CONTROL, wTopBase, SET_UVALUE = $
      { wEndAnimationButton: wEndAnimationButton, $      ; End button
        ...
        wRetreatOneButton: wRetreatOneButton, $          ; Retreat one frame
        wAdvanceOneButton: wAdvanceOneButton, $          ; Advance one frame
