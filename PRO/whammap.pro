...
;                > 0 < (!d.table_size-2-cbottom)
            ENDIF ELSE BEGIN 
              ...
              ;;;;; Original WHAM code for smooth=2:
...
;                           fvalue = f
            ...
            ;;;;; My modifed code for smooth=2. Delete 5 ; and comment out above to restore.
;;;;;;              f = bimage
;;;;;;              
;;;;;;;              triangulate, x, y, triangles, $
;;;;;;;                sphere = s, fvalue = f, /degrees
...
;          map = map[nozerobeams]
          ...
          map = map[nozerobeams]
          beamradius = beamradius[nozerobeams]
        ENDIF 
    ...
    IF NOT keyword_set(nolatborder) THEN BEGIN 
      ...
      oplot, intarr(1001)-(maxlon - 1e-2), $
             minlat+lindgen(1001)*dlat/1000.0, /noclip,color=254
               ...
               oplot, intarr(1001)-(linelon+ 1e-2), $
               linelat+lindgen(1001)*dlat/1000.0, /noclip,color=254
