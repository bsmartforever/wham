...
PRO lv, map, slice, width = width, lat = latvs, lonvs = lonvs, $
        ...
        vrange = vrange, crange = crange, noerase = noerase, _extra = extra

  IF NOT keyword_set(latvs) AND NOT keyword_set(lonvs) THEN BEGIN
    message, 'Please specify either /LAT or /LON for a LAT vs. VEL or LON vs. VEL plot', /info
    return
  ENDIF
  ...
  IF keyword_set(full) THEN BEGIN 
    IF keyword_set(lonvs) THEN BEGIN 
      minc = 0
      maxc = 360
      if n_elements(lti) then yti = lti else yti = 30
      if n_elements(lmi) then ymi = lmi else ymi = 3
      ytf = 'lonfullticks'
    ENDIF ELSE BEGIN 
      minc = -90
      maxc = +90
...
;    d =  data
    ...
    ;; if full requested for lv, transform to l=120 center
    IF keyword_set(full) AND keyword_set(lonvs) THEN BEGIN 
      y = y+60
      big = where(y GT 360)
      y[big] = y[big]-360
    ENDIF
...
;        ly = round((ly - !y.crange[0]) * yscale + py[0])
  ...
  plot, xax, yax, /nodata, xstyle = 1, ystyle = 1, /noerase, $
        ...
        ytickinterval = yti, yminor = ymi, ytickformat = ytf, $
