...
IF size(v,/type) eq 7 THEN begin
  root=v
  undefine,v
endif

        IF (N_PARAMS() EQ 0) AND (NOT keyword_set(root)) THEN BEGIN 
          print,' '
...
;
	IF (N_PARAMS() EQ 0) AND (keyword_set(root)) THEN BEGIN
		reflag = 1
		root = ' '
	ENDIF
	GOTO,LOOP4
