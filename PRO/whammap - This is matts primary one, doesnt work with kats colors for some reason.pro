PRO whammap, map, vcenter, vwidth, image, smimage, bimage = bimage, $
             ...
             coordbounds=coordbounds, mimage = mimage, tv = tv, $
             pointmin=pointmin, pointmax=pointmax, mollweide=mollweide, $
             _extra = extra, charsize=charsize,charthick=charthick,$
             magellanic=magellanic, gal2mag=gal2mag, xytitles=xytitles,$
             glondel=glondel,glatdel=glatdel, widthplot=widthplot
...
;   pointmin & pointmax: indices of blocks used for /fill.
;       Default is appropriate for blocks on the regular survey grid; need to 
;           specify if map is on blocks > 2000
;-
...
;                    1 / cos((abs(maxlat) > abs(minlat))/!radeg)) < 360
  ...
  IF keyword_set(multi) THEN BEGIN 
    map_set, latcenter, -loncenter, /hammer, $
      ...
      /advance
  ENDIF ELSE BEGIN
    IF keyword_set(mollweide) THEN $
        map_set, latcenter, -loncenter, /mollweide, $
          limit = [minlat, -maxlon, maxlat, -minlon], $
          /noborder, /iso, title = title, charsize = scale, $
          _extra = extra ELSE $
        map_set, latcenter, -loncenter, /hammer, $
    ...
    ENDIF ELSE BEGIN 
      ...
      IF keyword_set(smooth) THEN BEGIN 
        ...
        IF n_elements(missing) EQ 0 THEN BEGIN 
          IF n_elements(background) NE 0 THEN missing = byte(background) ELSE $
        ...
        IF keyword_set(fill) THEN BEGIN 
         IF n_elements(full) EQ 0 OR n_elements(map) LT 37565 THEN BEGIN 
            fillmap, mlon, mlat, bimage, missing, pointmin=pointmin, $
            	pointmax=pointmax
         ENDIF ELSE BEGIN 
...
;                > 0 < (!d.table_size-2-cbottom)
            ENDIF ELSE BEGIN 
              ...
              ;; GRIDDATA works much better by smoothing the original data first and then
              ;; bytescaling/clipping.
              ...
              if keyword_set(log) then begin
                ...
                ;; histogram equalize should be done before gridding to avoid high
                ;; latitude points gaining unequal influence in the output image
                ;; as input beams are stretched along rows.
                f = hist_equal(logimage, top = !d.table_size-2-cbottom)
              endelse
              
              qhull, x, y, triangles, sphere = tsphere, /delaunay
              
              gd = griddata(x, y, f, /kriging, search_ellipse = 4, $
                 tri = triangles, min_points = 10, $
                 xout = xout, yout = yout, /grid, /sphere, /degrees)
              
;              gd_missing = where(gd eq -1, gdm_cnt)
;              if gdm_cnt ne 0 then gd[gd_missing] = missing
              
              ;; Now bytescale and clip to limits
              IF keyword_set(log) THEN BEGIN
                smimage = bytscl(gd, $
                  max = alog10(zmax), min = alog10(zmin), $
                  top = !d.table_size-2-cbottom) $
              ...
              ENDIF ELSE IF keyword_set(linear) THEN BEGIN
                smimage = bytscl(gd, $
                  ...
                  + cbottom
              ENDIF ELSE BEGIN
                smimage = byte(gd > 0 < !d.table_size-2-cbottom) + cbottom
;                smimage = byte(gd, bottom = 0, top = !d.table_size-2-cbottom) + cbottom
              ENDELSE
...
;            print, 'making contour map'
            ...
            y = mlat
            z = image
            contour, z, x, y, /irr, /overplot, _extra = extra
            break
          END
        ENDSWITCH
...
;            polyfill, a*!radeg, mlat[i]+beamradius[i], color = cimage[i]
  ...
  FOR lon = glonmax, glonmin, -glondel DO BEGIN
    IF NOT keyword_set(nogrid) THEN $
      oplot, intarr(1000)-lon, $
        ...
        oplot, [299.183,193.169],[-38.9616,-44.6147],LINESTYLE = 5, THICK = 20
  ...
  IF NOT keyword_set(noframe) THEN BEGIN 
    ...
    ENDIF 
  ENDIF 
  IF keyword_set(widthplot) THEN BEGIN
