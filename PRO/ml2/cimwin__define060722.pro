    ...
    if ptr_valid(conbase_uval.cconfigs_ptr) then begin
        if obj_valid(*conbase_uval.cconfigs_ptr) then begin
            if obj_isa(*conbase_uval.cconfigs_ptr, 'CConfigs') then begin
                ...
                xsize=configs->GetDrawXs()
                ysize=configs->GetDrawYs()
            endif else begin
...
; disable digital filters for 2D images
dfilter_idx=(where(cimwin_desc.name eq 'Digital Filter'))[0]
dfilter_id=menu+dfilter_idx+1
widget_control, dfilter_id, sensitive=(im_zs gt 1)
...
; make the digital filter apply filter menu insensitive to start

recent_file_idx=(where(cimwin_desc.name eq 'Apply digital filter'))[0]
recent_file_index=recent_file_idx+2
recent_file_list=cimwin_desc[recent_file_index:recent_file_index+4].name

widget_control, menu+recent_file_index, sensitive=0
widget_control, menu+recent_file_index+1, sensitive=0
widget_control, menu+recent_file_index+2, sensitive=0
widget_control, menu+recent_file_index+3, sensitive=0
widget_control, menu+recent_file_index+4, sensitive=0
...
ext_arr_ptr=ptr_new(ext_arr, /allocate_heap)
...
if (conbase_uval.actwin_num_axes ne self.NAxis) then begin
    ...
    if (self.NAxis eq 3) then begin
        ; check the z limits on the new image
        if (conbase_uval.actwin_zmin gt im_s[2]) then $
          self.zMin=im_s[2]-1 else self.zMin=conbase_uval.actwin_zmin
        if (conbase_uval.actwin_zmax gt im_s[2]) then $
          self.zMax=im_s[2]-1 else self.zMax=conbase_uval.actwin_zmax
        ; update cube params
        widget_control, uval.wids.cube_curzs, set_value=strtrim(im_s[2], 2)
        ; update current range limits
        widget_control, uval.wids.cube_range_min, set_value=strtrim(self.ZMin, 2)
...
if (itime ne 0) then begin
    if (coadds ne 0) then begin
        case self.current_display of
            ...
            'As DN/s': begin
                if (self.current_display_update) then begin
                    if (uval.current_display_skip eq 0) then begin
                        im2=im2/(itime*coadds)
                    endif else begin
                        uval.current_display_skip=0
                        widget_control, self.BaseId, set_uval=uval
...
new_xs=(x1-x0+1)*self.XScale
if (new_xs lt 2) then new_xs=2

new_ys=(y1-y0+1)*self.YScale
if (new_ys lt 2) then new_ys=2
...
if (keyword_set(noerase) ne 1) then erase
...
if (self.naxis eq 3) then begin
    ...
    non_wavelength_index=where(im_s[*] ne wavelength_length)

    ; get the wavelength solution
    p_wavesol=self->WaveSol(wavelength_length)
    ; make sure this ptr is valid
    ...
    axes_values[0] = p_wavesol
;    axes_values[1] = ptr_new(findgen(im_s[non_wavelength_index[0]])*float(platescale))
;    axes_values[2] = ptr_new(findgen(im_s[non_wavelength_index[1]])*float(platescale))

    axes_values[1] = ptr_new(findgen(im_s[1])*float(platescale))
    axes_values[2] = ptr_new(findgen(im_s[0])*float(platescale))
    
    ; axes_titles=*(cconfigs->GetAxesLabels3d())
    axes_titles = ["Wavelength (!7l!3m)", "Y (arcsec)", "X (arcsec)"]
...
function CImWin::WaveSol, wavelength_length
...
/exclusive, /row, label_left='Median Nearest Neighbors:')
...
if (itime ne 0) then begin
    if (coadds ne 0) then begin
        ...
        case self.current_display of
            'As Total DN': begin
                ; check to see if this is the first frame opened
                instr=sxpar(hd,'CURRINST')
                if (instr ne 0) then begin
...
if ((self.XScale ge 1e-9) and (self.XScale lt 1e-2)) then begin
    xscale=sigfig(self.XScale, 2, /scientific)
    widget_control, base_uval.wids.zoom_xscl, set_value=xscale[0]
endif
if ((self.XScale ge 1e-2) and (self.XScale lt 1.)) then begin
...
if ((self.YScale ge 1e-9) and (self.YScale lt 1e-2)) then begin
    yscale=sigfig(self.YScale, 2, /scientific)
    widget_control, base_uval.wids.zoom_yscl, set_value=yscale[0]
endif
if ((self.YScale ge 1e-2) and (self.YScale lt 1.)) then begin
...
if ((self.DispMin le -1e-2) and (self.DispMin gt -1.)) then begin
    dispmin=sigfig(self.DispMin, 3)
    widget_control, base_uval.wids.stretch_min, set_value=dispmin[0]
endif 
if ((self.DispMin le -1e-9) and (self.DispMin gt -1e-2)) then begin
...
if ((self.DispMin ge 1e-9) and (self.DispMin lt 1e-2)) then begin
    dispmin=sigfig(self.DispMin, 2, /scientific)
    widget_control, base_uval.wids.stretch_min, set_value=dispmin[0]
endif
if ((self.DispMin ge 1e-2) and (self.DispMin lt 1.)) then begin
    dispmin=sigfig(self.DispMin, 3)
    widget_control, base_uval.wids.stretch_min, set_value=dispmin[0]
endif
...
if ((self.Dispmax le -1e-2) and (self.Dispmax gt -1.)) then begin
    dispmax=sigfig(self.Dispmax, 4)
    widget_control, base_uval.wids.stretch_max, set_value=dispmax[0]
endif
if ((self.Dispmax le -1e-9) and (self.Dispmax gt -1e-2)) then begin
...
if ((self.DispMax ge 1e-9) and (self.DispMax lt 1e-2)) then begin
    dispmax=sigfig(self.DispMax, 2, /scientific)
    widget_control, base_uval.wids.stretch_max, set_value=dispmax[0]
endif
if ((self.DispMax ge 1e-2) and (self.DispMax lt 1.)) then begin
...
pro CImWin::Logarithmic, base_id
...
pro CImWin::DiagonalCut, base_id
...
; realize plot window
plot_win=obj_new('CPlotWin', ParentBaseId=self.BaseId, $
    p_DispIm=self.p_DispIm, p_Im=im_ptr, type='diagonal', $
    win_backing=base_uval.win_backing)
...
base_uval.exist.plot=plot_win->GetBaseId()
; set box mode
base_uval.box_mode='diagonal'
widget_control, base_id, set_uval=base_uval

end

pro CImWin::Surface, base_id, display_gaussian
...
; set box mode
if NOT keyword_set(display_gaussian) then $
...
; get subimage
    ...
    case self.fit_type of 
        0: begin
            ...
            yfit=mpfit2dpeak(array, coeff, x_arg, y_arg)
        end
        1: begin
...
pro CImWin::SetCurrentDisplay, new_current_display
...
function CImWin::GetCurrentDisplayUpdate
return, self.current_display_update
end

pro CImWin::SetCurrentDisplayUpdate, new_current_display
self.current_display_update=new_current_display
end
...
cprint_ptr=ptr_new(/allocate_heap)
cdfilter_ptr=ptr_new(/allocate_heap)
cfitshedit_ptr=ptr_new(/allocate_heap)
unraveled_locations_ptr=ptr_new(/allocate_heap)
