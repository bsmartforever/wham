...
cimwin_desc = [{cw_pdmenu_s, 1, 'File'}, $
               ...
               {cw_pdmenu_s, 0, 'Sqrt'}, $
               {cw_pdmenu_s, 0, 'Position Angle'}, $
               {cw_pdmenu_s, 2, 'As Total DN'}, $
               {cw_pdmenu_s, 1, 'Tools'}, $
...
tv,dispim,win_xs/2-((uval.tv_p0[0]-x0)*self.XScale)-1, $
...
if pa[0] eq -1 then begin
    ...
    if (self.naxis eq 3) then begin
        case self.axesorder[0] of
            1: begin
                if (self.axesorder[1] eq 2) then begin
                    ; 270 rotation + flip
                    pa[0]=pa[0]+270
                    ; check to make sure you haven't rotated past 360
                    if (pa[0] ge 360) then pa[0]=pa[0]-360
                    ; find out which quadrant N is in           
                    if ((pa[0] ge 0) and (pa[0] le 90)) then quad=1
                    if ((pa[0] gt 90) and (pa[0] le 180)) then quad=2
                    if ((pa[0] gt 180) and (pa[0] le 270)) then quad=3
                    if ((pa[0] gt 270) and (pa[0] le 360)) then quad=4
                    ; make the quadrant flip
                    case quad of
                        1: begin
                            ang1=pa[0]
                            ang2=180-ang1
            ...
            2: begin
                ; 2,1,0 is no rotation
                if (self.axesorder[1] ne 1) then begin
                    answer=dialog_message(['PA not available in this orientation.'], $
                                          /error, dialog_parent=base_id)
                    return
                endif
        ...
        case self.axesorder[0] of
            ...
            1: begin
                if (self.axesorder[1] eq 0) then begin
                    ; 270 rotation + flip
                    pa[0]=pa[0]+270
                    ; check to make sure you haven't rotated past 360
                    if (pa[0] ge 360) then pa[0]=pa[0]-360
    ...
    one_arrow, x, y, pa[0]+90, 'N', arrowsize=[float(l), l/5.0, 35.0], $
      color=mask, charsize=3
    one_arrow, x, y, pa[1]+90, 'E', arrowsize=[float(l), l/5.0, 35.0], $
      color=mask, charsize=3
    device, set_graphics=oldg
    wset, save
endelse
...
ql_aper, disp_im, cimwin_uval.phot_circ_x, cimwin_uval.phot_circ_y, flux, eflux, sky, skyerr, 1, self.photometry_aper, $
  [self.photometry_inner_an, self.photometry_outer_an], [0,0], numaper_pix=numaper_pix, /FLUX
...
if (numaper_pix gt 0) then begin
    ...
    message='Please select an aperture that is within the limits of the image.'
    answer=dialog_message(message, dialog_parent=self.BaseId, /error)
    return
endelse    
...
if (uval.exist.plotwin ne 0L) then begin
    ...
    message='Please select atleast a 5x5 box for Gaussian fitting'
