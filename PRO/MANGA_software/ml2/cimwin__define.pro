...
; 2013-05-29: DRL hacking this to handle MaNGA log wavelength solution
...
crpix3=sxpar(hd,'CRPIX3')
crval3=sxpar(hd,'CRVAL3')
cd3=sxpar(hd,'CD3_3')
ctype3=strcompress(sxpar(hd,'CTYPE3'),/remove_all)

; MaNGA should have WAVE-LOG type wave solution
if (ctype3 eq 'WAVE-LOG') then begin
  wavelength_length=sxpar(hd,'NAXIS3')
  wave10=crval3+cd3*(findgen(wavelength_length)-crpix3)
  logstart_wavelength=crval3
  ; Convert to linear microns
  start_wavelength=10.^logstart_wavelength/10000.
  wave_solution=10.^wave10/10000.
  p_wavesol=ptr_new(wave_solution)

  ; Some keywords
  im_s=([ImObj->GetXS(), ImObj->GetYS(), ImObj->GetZS()])
  lambdact=wavelength_length
  cunit="micron"
  wavelength_index=2
 return, p_wavesol
endif
; If LOG10 keyword wasn't set, use default OSIRIS keyword behaviour
