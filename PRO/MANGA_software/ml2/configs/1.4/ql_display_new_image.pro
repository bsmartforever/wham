        ...
        if obj_valid(*p_WinObj) then begin
            if obj_isa(*p_WinObj, 'CImWin') then begin
                ...
                if (im_zs gt 1) then begin
                    widget_control, winbase_id, get_uval=winbase_uval
                    widget_control, winbase_uval.wids.dfilter_id, sensitive=1 
                    ...
                    if (winbase_uval.exist.plot ne 0L) then begin
                        ...
                        widget_control, plot_uval.wids.plot_type_menu, $
                                        set_value=type_list
                        ; update the type list in the plotwin uval
                    ...
                    widget_control, winbase_id, get_uval=winbase_uval
                    widget_control, winbase_uval.wids.dfilter_id, sensitive=0
                ...
                imwin_obj->MakeExtArr, im_extensions, ext_arr, ext_arr_ptr
...
if ((des ne 'As Total DN') or (des ne 'As DN/s')) then begin
    des=['As Total DN']
endif
...
if (des eq 'As DN/s') then begin
    ...
    if (itime ne 0) then begin
        if (coadds ne 0) then begin
            im=*self.p_DispIm
            im=*(imwin_obj->GetDispIm())
            
            im=im*itime*coadds
            *(imwin_obj->GetDispIm())=*im
        endif else begin
