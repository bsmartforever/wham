@~madsen/bin/xloadct
@~madsen/bin/intmap
@/d/wham2/madsen/pro/whammap
@/d/wham2/madsen/pro/xinteranimate
@/d/wham2/madsen/pro/cw_animate
@/d/wham2/madsen/pro/intmap_fast
@/d/wham/pro/greg_sparith
...
PRO WHAMVIEW
  ...
  Par_Flags = { Map_Path:"/d/wham3/survey/", Rcd_Path:"~", $
  ...
  ;; Defined here to avoid error messages (yes, this is ugly)
  ...
  CASE Event.ID OF
        ...
        CASE buttonUValue[Event.Index] OF
           ...
           'HI': BEGIN
              ...
              IF Bad_ID EQ 0 THEN BEGIN
                 ...
                 ;; this auto-selects 'no conv.' in options window
     ...
     Widget_Window_1_Buttons.Grab_Map:BEGIN
        ...
        IF Par_Flags.Use_HI THEN BEGIN
           ...
           ;;  VMin = -500 &  VMax = 500
           ;;ENDIF
           ;; Change this to always grab +/- 500 km/s of data
           Map = EXTHI(LMin, LMax, BMin, BMax, vmin = -500, vmax = 500, /wham)
        ENDIF
                 ...
                 IF Count GT 0 THEN BEGIN
                    ...
                    IF NOT(Par_Flags.Use_HI) AND NOT(Par_Flags.Use_CO) AND $
                       ...
                       FOR i = 0, Count-1 DO BEGIN
                          Star_Ind = WHERE(SPHDIST(SAO6.Glon, SAO6.Glat, $
                                                   Map[Near_Inds[i]].Glon, Map[Near_Inds[i]].Glat,  /deg) $
                                           LE 0.55, Star_Count)
              ...
              ENDIF ELSE BEGIN 
                 ...
                 ENDIF ELSE IF (Event.Release EQ 1) THEN BEGIN 
                    IF Par_Flags.Use_HI OR Par_Flags.Use_CO THEN BEGIN
                       ...
                       ;;ENDIF
                       ;; Changed this to display +/- 125 km/s nr VCen
                       VMin = V_Cen-125 &  VMax = V_Cen+125
...
PRO Map_Options,  Parent
  ...
  ;; defined way up top to avoid error messages
;;  Options_Widget = {Main:0L, Lin_Log:0L, Convert:0L, Other:0L, $
;;                    V_Start:0L, V_End:0L, Step:0L, Make_Movie:0L, $
;;                    CT:0L, Quit:0L}
