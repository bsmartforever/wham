...
dir='/d/wham/bsmart/data/hi/Nidever_Stream/'
;
	  ...
	  while(i le subnum_good-1) do begin
		...
		;******* Sets colors for block depending on ZD_start_of_night
	  	...
	  	if keyword_set(blocks) AND (npoints ge 15) then begin
			...
			PLOTs, -[mlon[pointings[0]]-.5,mlon[pointings[n-1]]+.5], [mlat[pointings[0]]-.5,mlat[pointings[0]]-.5],  color=c
			PLOTs, -[mlon[pointings[0]]-.5,mlon[pointings[n-1]]+.5], [mlat[pointings[n-1]]+.5,mlat[pointings[n-1]]+.5],  color=c
			PLOTs, -[mlon[pointings[n-1]]+.5,mlon[pointings[n-1]]+.5], [mlat[pointings[0]]-.5,mlat[pointings[n-1]]+.5],  color=c
			PLOTs, -[mlon[pointings[0]]-.5,mlon[pointings[0]]-.5], [mlat[pointings[0]]-.5,mlat[pointings[n-1]]+.5],  color=c
	  	endif
		...
		if npoints ge 15 then begin
      		xyouts,-avg(mlon[pointings]),avg(mlat[pointings])-0.5,$
      			string(fix(bnum[i],type=2)),color=cgcolor('black'),$
